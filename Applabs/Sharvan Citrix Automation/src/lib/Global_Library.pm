...
sub ftp_put {
	...
	$me->print_message("Putting file [$remote_file] on FTP Server");
	if ($ftp->put($file, $remote_file)) {
		$me->print_message("Put [$remote_file] successfully");
		$ftp->quit;
		return 1;
	}else {
		$me->print_message("Couldn't put [$remote_file]");
		$ftp->quit;
		return 0;
	}
}
...
sub wget_http {
	...
	my ($sec1, $usec1) = gettimeofday();
	$me->print_message("Getting file [$url] from HTTP Server to [$local_file]");
	if (!system("wget --output-document $local_file $url")) {
		$me->print_message("Got successfully");
		my($sec2, $usec2) = gettimeofday();
		my $sec = ($sec2-$sec1)+(((1000000-$usec1)+$usec2)/1000000);
		return ($sec);
...
sub share_windows {
	...
	if (!system("net share $share_name > $script_log 2>$script_error_log")) {
		...
		if (!system("net share $share_name \/delete > $script_log 2>$script_error_log")) {
			$me->print_message("Deleted existing share [$share_name] successfully");
		}
		else {
			my $system_error=$me->get_error($script_error_log);
...
sub get_trans_compression_ratio{
	shift;
	my $ws = shift;
	
	my $param1 = "CompressionClearTextBytes";
	my $param2 = "CompressionCipherTextBytes";

	$me->print_message("Calculating compression value");
	my $response1 = XMLRPC::Lite->proxy("http://$ws:2050/RPC2")
		->call('Get', {Class => "SYSTEM", Attribute => $param1})
		->result;
        
	my $response2 = XMLRPC::Lite->proxy("http://$ws:2050/RPC2")
		->call('Get', {Class => "SYSTEM", Attribute => $param2})
		->result;
        
	if ( (element_exists(${$response1}{$param1})) && ( exists(${$response1}{$param1}{'Fault'})) ) {
	      $me->print_message("Fault returned by get_parameter");
	      $me->print_message(%{$response1});
	      return 0;
	}
	if ( (element_exists(${$response2}{$param2})) && ( exists(${$response2}{$param2}{'Fault'})) ) {
	      $me->print_message("Fault returned by get_parameter");
	      $me->print_message(%{$response2});
	      return 0;
	}
	
	#print $response1->{$param1}->{"Total"};
	#print "\n";
	#print $response2->{$param2}->{"Total"};
	my $ratio = sprintf("%.1f", $response1->{$param1}->{"Total"}/$response2->{$param2}->{"Total"});
	#print "\n";
	#print $ratio."\n";
	if($ratio >=1) {
		$me->print_message("Compression value is $ratio");
		return $ratio;
	}else{
		$me->print_message("Compression value is $ratio");
		return undef;
...
sub get_recv_compression_ratio{
	shift;
	my $ws = shift;
	
	my $param1 = "DecompressionClearTextBytes";
	my $param2 = "DecompressionCipherTextBytes";
...
sub verify_compression_ratio {
	shift;
	$first_pass = shift;
	$second_pass = shift || undef;
	
	$me->print_message("Verifying compression ratio");
	if ($second_pass eq undef) {
		if ($first_pass > 1) {
			$me->print_message("First Pass Compression Ratio : 1:$first_pass");
			return 1;
		}else {
			$me->print_message("First Pass Compression Ratio : 1:$first_pass");
			return 0;
	...
	if ($first_pass > 1) {
			if ($second_pass > $first_pass) {
				$me->print_message("First Pass Compression Ratio : 1:$first_pass");
				$me->print_message("Second Pass Compression Ratio : 1:$second_pass");
				return 1;
			}else {
				$me->print_message("First Pass Compression Ratio : 1:$first_pass");
				$me->print_message("Second Pass Compression Ratio : 1:$second_pass");
