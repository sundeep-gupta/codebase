...
#
# Script ID:			MCR-179
# Script Name:			MCR-179.pl
# Description:			Verifies compression-ratio of non-compressible-data transferred using wget FTP in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-179.pl [--size  <size of file>
...
my ($file, $source_checksum, $target_checksum, $ctr, $first_pass, $second_pass);
...
my $url = "ftp://".$server."/ftp-path/$TYPE_OF_FILE/$file";
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	#Transfer file
	if ((!$remote_library->wget_ftp($client, $url, $ftpusr, $ftppasswd, "$share_path/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
	...
	if ((!$remote_library->delete_file($client,"$share_path/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
	#Transfer file
	if ((!$remote_library->get_ftp($client, $server, $ftpusr, $ftppasswd, "/$TYPE_OF_FILE/$file", "$share_path/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
	#Calculate checksum at destination
	if (($target_checksum = $remote_library->calculate_checksum($client,"$share_path\\$TYPE_OF_FILE","$file$ctr")) eq undef) {
