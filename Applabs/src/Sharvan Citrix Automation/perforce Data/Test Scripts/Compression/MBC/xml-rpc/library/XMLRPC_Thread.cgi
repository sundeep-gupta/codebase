...
sub put_ftp_using_threads {
	...
	for ($ctr=0;$ctr<=$#source_files;$ctr++) {
		if (!defined($pid = fork())) {
			...
			if ($ftp->put("$source_dir/$source_files[$ctr]","ftp-path$target_dir/$target_files[$ctr]")) {
				$ftp->quit;
			}else {
				$ftp->quit;
				kill 9, @cpids;
...
sub get_ftp_using_threads {
	...
	for ($ctr=0;$ctr<=$#source_files;$ctr++) {
		if (!defined($pid = fork())) {
			...
			if ($ftp->get("ftp-path$source_dir/$source_files[$ctr]","$target_dir/@target_files[$ctr]")) {
				$ftp->quit;
			}else {
				$ftp->quit;
				kill 9, @cpids;
