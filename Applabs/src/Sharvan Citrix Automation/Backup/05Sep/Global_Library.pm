...
sub check_matching_file {
	...
	while(defined ($file = readdir DIR)){
		if (-s "$path\\$file" == $size) {
			$me->print_message("Matching file found");
			return $file;
		}
	}
...
sub map_share {
	...
	foreach (@drives) {
		...
		while ($count <= 3) {
			...
			if ($res == 0) {
				$me->print_message("Mapped the drive successfully");
				return "$_";
			}
			sleep(10);
			$count = $count+1;
...
sub create_file {
	...
	if (open(FH, ">$path\\$file_name")){
		...
		close(FH);
		$me->print_message("File created successfully");
		return $file_name;
	}
	else{
