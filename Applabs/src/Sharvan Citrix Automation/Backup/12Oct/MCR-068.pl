...
# 
# Script ID:			MCR-068
# Script Name:			MCR-068.pl
# Description:			Verifies integrity of mid-compressible-data transferred using wget http in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-068.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->wget_http_using_threads($client, $url, "/$TYPE_OF_FILE",\@source_files,"$share_path/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
}
