...
# 
# Script ID:			MCR-095
# Script Name:			MCR-095.pl
# Description:			Verifies integrity of non-compressible-data transferred using CIFS in multi-session [different set of data] from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-095.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->copy_files_using_threads($client,"$share_path/$TYPE_OF_FILE",$ref_files, "//$server/$share/$TYPE_OF_FILE",$ref_files))) {
	exit 2;
}
if ((!$remote_library->delete_files_using_threads($server,"$share_path/$TYPE_OF_FILE",$ref_files))) {
	exit 2;
}
if ((!$remote_library->copy_files_using_threads($client,"$share_path/$TYPE_OF_FILE",$ref_files, "//$server/$share/$TYPE_OF_FILE",$ref_files))) {
	exit 2;
