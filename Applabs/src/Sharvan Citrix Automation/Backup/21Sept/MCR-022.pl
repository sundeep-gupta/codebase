...
# 
# Script ID:			MCR-022
# Script Name:			MCR-022.pl
# Description:			Verifies integrity of transferred data at first level of compression level 2
...
if ($help) {
	print "Usage: #perl mcr-022.pl [ --stype <type of system, script is running on>
...
#Transfer file
if (($transfer_time = $global_library->ftp_put($server, $ftpusr, $ftppasswd, "$working_folder$TYPE_OF_FILE/$file")) eq undef) {
	exit 2;
}
