...
if ($help) {
	print "Usage: #perl mcr-009.pl [ --stype <type of system, script is running on>
...
if ((!$remote_library->wget_http($client, $url, $ftpusr, $ftppasswd,"$share_path/$TYPE_OF_FILE/$file"))) {
	exit 2;
}
