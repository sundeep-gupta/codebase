...
# 
# Script ID:			MCR-109
# Script Name:			MCR-109.pl
# Description:			Verifies compression-ratio of compressible-data transferred using CIFS from client to server at first level of compression
# Pass/Fail Criteria:		Pass if compression-ratio is accurate, otherwise Fail.
...
if ($help) {
	print "Usage: #perl mcr-109.pl [ --size  <size of file>
...
my ($file, $source_checksum, $target_checksum, $first_pass);
...
#Transfer file
if ((!$remote_library->copy_file($client,"$share_path/$TYPE_OF_FILE/$file", "//$server/$share/$TYPE_OF_FILE"))) {
	exit 2;
}
