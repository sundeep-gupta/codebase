...
Dong Duong 
8/23/05
This test verifies the flowcontrol at 500Mbps
9/9/05 (dong)
Modify the stat header to incluse the LOSS
=cut 
...
if (  ! $opt_s || ! $opt_o || !$opt_r) {
  print "Usage: $testcase -o <locOrb> -r <remOrb> -s <Iperf Server> |-c <1> |-m <1> |-b <1>   \n" ;
  exit 1 ;
}
...
#   print `mail \-s \"Release $release: 500Mbps Performance\" engineering\@orbitaldata.com \< $stat_file`;
    print `mail \-s \"Orb-6500 Release $release: 500Mbps Performance\" dong\@orbitaldata.com \< $stat_file`;
   }
