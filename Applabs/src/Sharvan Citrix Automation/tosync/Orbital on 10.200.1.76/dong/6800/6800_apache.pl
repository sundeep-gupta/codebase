...
if ( !$opt_s || !$opt_o || !$opt_r) {
  print "\nUsage: $testcase.pl -o <locOrb> -r <remOrb> -s < HTTP Servers> | -b <1> | -m <1>| -d <dr IP> \n\n";
  exit 1;
}
...
if ($send_mail) {
   print `mail \-s \"Orb6800 ApacheBench Test Results of Release $rel\" engineering\@orbitaldata.com  \< $log_file`;
