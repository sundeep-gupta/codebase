...
=head1 DESCRIPTION
Verify that the system stays up for at least 24 hrs of reliability test
...
Dong Duong 
09/08/2005
Verify the reliability of Orb 6800
=cut 
...
my $test_case='reliability.pl'

getopt ("r, o, s, e") ;
if (  !$opt_s && !$opt_e && !$opt_o && !$opt_r  ) {
  print "Usage:  $test_case -o <locOrb> -r <remOrb> -s <rem host> -e <non_acc host> \n" ;
  exit 1 ;
}
my $locOrb = $opt_o;
chomp ($locOrb);
