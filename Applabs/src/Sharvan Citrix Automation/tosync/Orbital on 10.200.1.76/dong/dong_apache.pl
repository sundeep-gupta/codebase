...
my $pod4_dr = '10.200.199.154';       #default for POD4 testbed
my $dr_dest = '0.0.0.0';
my $dr_bw = 100000;          #100Mbps
my $dr_rtt = 100;
my $dr_loss = 1;
...
#Log file will be stored in /logs/tmp
   my $log_file = "\/logs\/TestResult\/"."$testcase"."_$tm"."_$rel".".log";
...
#Set DR 
config_CNISTNET ($dong_dr, $dr_dest, $dr_bw, $dr_rtt, $dr_loss);
...
# 
config_CNISTNET ($dong_dr, $dr_dest, $dr_bw, 0,0);
...
#Change the  DR back to its original default value 
config_CNISTNET ($dong_dr, $dr_dest, $dr_bw, $dr_rtt, $dr_loss);
...
#----------------------------------------------------------
  print "Turn Compression OFF ", $orb_rpc->set_parameter('Compression.EnableCompression', '0'), "\n";
  print "Set The Tested BW to $bw_Mbps \n ", $orb_rpc->set_parameter('SlowSendRate',"$bw_bps");
  sleep 3;
