...
# 
# Script ID:			MCR-070
# Script Name:			MCR-070.pl
# Description:			Verifies integrity of non-compressible-data transferred using FTP in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl cr-070.pl [ --stype <type of system, script is running on>
...
#Transfer file
if (($transfer_time = $global_library->ftp_get($server, $ftpusr, $ftppasswd, $file, "$working_folder$file")) eq undef) {
	exit 2;
}
...
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	if (!defined($pid = fork())) {
		...
		if (($transfer_time = $global_library->ftp_get($server, $ftpusr, $ftppasswd, $file, "$working_folder$file$ctr")) eq undef) {
			kill 9, $ppid;
			kill 9, @cpids;
			exit 2;
		}
