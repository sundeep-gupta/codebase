...
# 
# Script ID:			MCR-094
# Script Name:			MCR-094.pl
# Description:			Verifies integrity of mid-compressible-data transferred using FTP in multi-session [different set of data] from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-094.pl [ --stype <type of system, script is running on>
...
#Transferring files
if (!$thread_library->ftp_multi_put($server, $ftpusr, $ftppasswd, "$working_folder$TYPE_OF_FILE/",\@files)) {
	exit 2;
}
