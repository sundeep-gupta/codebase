...
if ($help) {
	print "Usage: #perl cr-067.pl [ --stype <type of system, script is running on>
...
my $url = "ftp://".$server."/ftp-path/$file";

#Transfer file
if (($transfer_time = $global_library->wget_ftp($url, $ftpusr, $ftppasswd, "$working_folder$file")) eq undef) {
	exit 2;
}
...
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	if (!defined($pid = fork())) {
		...
		if (($transfer_time = $global_library->wget_ftp($url, $ftpusr, $ftppasswd, "$working_folder$file$ctr")) eq undef) {
			kill 9, $ppid;
			kill 9, @cpids;
			exit 2;
		}
