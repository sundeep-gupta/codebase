...
#
# Script ID:			MCR-201
# Script Name:			MCR-201.pl
# Description:			Verifies compression-ratio of mid-compressible-data transferred using CIFS in multi-session [different set of data] from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-201.pl [--size  <size of file>
...
my ($file, $ref_source_checksums, $target_checksum, $ctr, $ref_files, $first_pass, $second_pass);
...
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	...
	$file=@{$ref_files}[$ctr];
	#Transfer file
	if ((!$remote_library->copy_file($client,"$share_path/$TYPE_OF_FILE/$file", "//$server/$share/$TYPE_OF_FILE/$file"))) {
		exit 2;
	}
