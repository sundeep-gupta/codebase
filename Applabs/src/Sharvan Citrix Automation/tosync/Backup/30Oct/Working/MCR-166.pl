...
#
# Script ID:			MCR-166
# Script Name:			MCR-166.pl
# Description:			Verifies compression-ratio of mid-compressible-data transferred using FTP in multi-session [same data] from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-166.pl [--size  <size of file>
...
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	...
	#Transfer file
	if ((!$remote_library->put_ftp($client, $server, $ftpusr, $ftppasswd, "$share_path/$TYPE_OF_FILE/$file","/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
	...
	if ((!$remote_library->delete_file($server,"$share_path/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
	if ((!$remote_library->put_ftp($client, $server, $ftpusr, $ftppasswd, "$share_path/$TYPE_OF_FILE/$file","/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
