...
Readonly my $IPERF_TEST		=> 0x08;
Readonly my $TEST			=> $CIFS_TEST | $FTP_TEST | $HTTP_TEST | 0x08;
...
foreach my $client_ip (@client_ips) {
    ...
    if(($TEST & $CIFS_TEST) == $CIFS_TEST) {
	    $cifs_threads[$i] = threads->new(\&do_cifs,
        								 $client_ip,
                                         \%cifs);
    ...
    if(($TEST & $FTP_TEST) == $FTP_TEST) {
		$ftp_threads[$i] = threads->new(\&do_ftp,
        								$client_ip,
                                        \%ftp);
    ...
    if(($TEST & $HTTP_TEST) == $HTTP_TEST) {
		$http_threads[$i] = threads->new(\&do_http,
        								 $client_ip,
                                         \%http);
    ...
    if(($TEST & $IPERF_TEST) == $IPERF_TEST) {
		$iperf_threads[$i] = threads->new(\&do_iperf,
        								 $client_ip,
                                         \%iperf);
