...
#	my $self = shift;
    ...
    my $result = $wanscaler->set_virtual_clients($options->{'virtual_clients'});
	return $result;
}
...
#	my $options = shift;
    my $wanscaler = WANScaler::XmlRpc->new("http://".LOCALHOST.':'.XMLRPC_PORT."/RPC2");
    my $result = $wanscaler->get_virtual_clients();
    print 'Library = '. Dumper($result);
	return $result;
}
...
sub start_wanscaler_service {
	my $status = {};
    # The below method is not foolproof as WS Client gives servie started status
    # even when it fails to do so. Try doing it thorugh Perl API [If possible].
    my @response = `net start wanscalerclient`;
#	GetStatus(LOCALHOST,WS_SERVICE_NAME,$status);
#	print Dumper($status);
#	StartService(LOCALHOST,WS_SERVICE_NAME);
#	GetStatus(LOCALHOST,WS_SERVICE_NAME,$status);
#	print Dumper($status);
...
#	GetStatus(LOCALHOST,WS_SERVICE_NAME,$status);
#	print Dumper($status);
#	StopService(LOCALHOST,WS_SERVICE_NAME);
#	GetStatus(LOCALHOST,WS_SERVICE_NAME,$status);
#	print Dumper($status);
}
1;