...
our $CAPTURE_CLIENT_METRICS 	= FALSE;
our $RESET_PERF_COUNTERS	= FALSE;

# CONFIG for WANScaler Client
our %wanscaler_client = (
			PARAMETER => {
                       #    	'Dbc.MaxPendingPerSpindle' 	=> 1,
  	               #	'System.VirtualClients' 	=> 10,
...
our %cifs = (
		ip_address          => [172,32,2,20],
		share_name 		=> 'Share',
		operation 		=> $CIFS_WRITE ,
		...
		'read'		=> {
					...
					#src_files 	=> ['file1.dat','file2.dat'],
					sessions 		=> 1,
#					read_to_null	=> TRUE,
                                        #ALWAYS WILL BE COPIED TO NUL.
				       },
		'write'		=> {
					src_file_path 	=> 'C:\\Scalability\\Script\\Scalability',
					src_files	=> 'ALL',
					sessions 	=> 1,
					dst_file_path	=> 'write\\',
				       },
		data_type 		=> $COMPRESSIBLE # COMPRESSIBLE -> 1 UNCOMPRESSIBLE -> 0
            );
our %ftp = (
            server 		=> '172.32.2.41',
