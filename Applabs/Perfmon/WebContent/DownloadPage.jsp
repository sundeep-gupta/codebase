...
try
 {
	...
	int serviceId = Integer.parseInt((String)session.getAttribute("serviceid"));
  ...
  String sql = "select serviceid,reporting.agentid,time,name,dnstime connecttime,ttfb,downloadtime,responsetime,status,responsecode from reporting,agents,cities where date_format(time, '%Y-%m-%d') >=\'" + date1 + "\' and date_format(time, '%Y-%m-%d') <=\'" + date2 + "\' and  reporting.agentid=agents.agentid and agents.cityid=cities.cityid and cities.cityid in " + cityID + " and serviceid=" + serviceId + " order by reporting.time, cities.name";
