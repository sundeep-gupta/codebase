...
<HTML>
<HEAD>
<TITLE>Monthly Detailed Report</TITLE>
...
var SelectedStatus;
/*
		function for displaying the Monthly Detailed Reports
*/
...
document.getElementById("one").src ="<%=request.getContextPath() %>/run?__report=MonthlyDetailedChartReport.rptdesign&sidParam=<%= serviceId %>&uidParam=<%= userId %>&cityParam=" + SelectedLocation;

	if(SelectedStatus=="All"){	
document.getElementById("two").src="<%=request.getContextPath() %>/frameset?__report=MonthlyDetailedTableReport.rptdesign&__overwrite=true&sidParam=<%= serviceId %>&uidParam=<%= userId %>&cityParam=" + SelectedLocation;
}
else{
//document.getElementById("one").src ="/NewIntegration/run?__report=MonthlyDetailedChartReport.rptdesign&sidParam=<%= serviceId %>&uidParam=<%= userId %>&cityParam=" + SelectedLocation;
document.getElementById("two").src="<%=request.getContextPath() %>/frameset?__report=MonthlyDetailedFailure.rptdesign&__overwrite=true&sidParam=<%= serviceId %>&uidParam=<%= userId %>&cityParam=" + SelectedLocation;
}	
}
/*
...
function monthDays(){
	var dt = new Date();
	dt.setDate(dt.getDate()-dt.getDate())
	mon = dt.getMonth()
	mon += 1
	var lastDate = mon + '-' + dt.getDate() + '-' + dt.getFullYear();
	var firstDate = mon + '-'+ '01'+'-' + dt.getFullYear();
	//var lastDate = dt.getDate() + '-' + mon + '-' + dt.getFullYear();
	//var firstDate = '01-' + mon + '-' + dt.getFullYear();
	labelVal = "From Date   : " + firstDate + ' To Date : ' + lastDate;
	document.getElementById('monthDate').innerText = labelVal;

}
/*
	Function for setting the title of Monthly Detailed Report
*/
function setTitle(){
...
%>
<BODY onLoad="javascript:monthDays(),setTitle()">
<form name="" method="get">
<table width="95%" border="0" cellpadding="0" cellspacing="0" align="center" class="tabinside">
			
				<table width="95%" border="0" cellpadding="0" cellspacing="0" align="center" class="tabinside">
        <tr> 
          <td height="30" class="tabheading">&nbsp;&nbsp;Detailed - Monthly</td>
        </tr>
        <tr> 
          <td  valign="top">
		<table width="95%" align="CENTER" border="0" cellspacing=0 cellpadding=0  height="40%">
					...
					<tr>
					<td id="monthDate" align colspan="3" class="fieldnames" height="30" width="55%">
					</td>
					</tr>
						<tr>
			...
			<td class="fieldnames" valign=middle height="30" width="20%" align=left><b>
			Select Status:
			...
			<td align="left" width="25%"><input type="button" value="View Report" class=button  onClick="javascript:generateMonthlyReports()"/></td>
						</tr>
