	...
	if ((update_data(%form_data)) eq 1)
	{
		my $template = HTML::Template->new(filename=>'response.tmpl');
	        print "Content-Type: text/html\n\n";
                $template->param(MESSAGE=>'Work Code has been updated!',);
...
{
    my $template = HTML::Template->new(filename=>'modifyWID.tmpl');
    my $id = '';
    my $name = '';
    my $active = '';
    my %form_fields = @_;    
    ...
    my $client_string = "SELECT CodeID, CodeName, CodeActive FROM workcode WHERE CodeID ='$chosen_code'";
    my $client_command = $db_connection->prepare($client_string);
    $client_command->execute or db_die($db_connection->errstr);
    $client_command->bind_columns(undef, \$id, \$name, \$active);
...
sub update_data
{
  	...
  	my $chosen_code = $form_fields{CodeID};
	#check parameter
        unless (%form_fields) 
        {
        ...
        my $update_string = "UPDATE workcode SET ";
        ...
        $update_string .= " WHERE CodeID='$chosen_code'";
