#!/usr/bin/perl -w
######################################################
#                 insert_log.cgi - PPTS :: ProTS
######################################################
# Copyright   :   (c)2000 Whetstone Logic, Inc.
# Date Started:   Feb. 20, 2000
...
if ((insert_data(%form_data)) eq 1)
{
	my $template = HTML::Template->new(filename=>'response.tmpl');
        ...
        $template->param(
                         PID=>$form_data{ProjectID},);
        print $template->output;
        exit;
} 
else 
{
        error('I could not insert the log entry.<br>Please contact administrator.');
}
...
sub insert_data
{
	...
	#check parameter
        ...
        my $insert_string = "INSERT INTO projectlog SET ";
