...
my $sth = $dbh->prepare("SELECT clientusername,sum(bytesrecvd+bytessent) as val FROM DailyUserWebProxyLog where DATE_FORMAT(logDate, '%H:%i') like '$last_time:%' group by ClientUserName order by val desc limit 10");
...
#####
my $sth = $dbh->prepare("SELECT DestHost,sum(bytesrecvd+bytessent) as val FROM DailyUserWebProxyLog where DATE_FORMAT(logDate, '%H:%i') like '$last_time:%' group by DestHost order by val desc limit 10");
if (!$sth) {die "Error:" . $dbh->errstr . "\n";}
if (!$sth->execute) {die "Error:" . $sth->errstr . "\n";}
...
if ( $time < 10 )
{
$c->addTitle("Top 10 Internet Users From 0$time:00 to 0$time:59 Hrs - Today", "arialbi.ttf");
}
else
{
$c->addTitle("Top 10 Internet Users From $time:00 to $time:59 Hrs - Today", "arialbi.ttf");
}
...
if ( $time < 10 )
{
$d->addTitle("Top 10 Internet Sites From 0$time:00 to 0$time:59 Hrs - Today", "arialbi.ttf");
}
else
{
$d->addTitle("Top 10 Internet Sites From $time:00 to $time:59 Hrs - Today", "arialbi.ttf");
}
...
<a href="http://172.25.10.121/cgi-bin/isa_current_database_report.pl?data=usersperhour&time=$time"><img src="http://172.25.10.121/excel.gif" border="0" align="top" width="16" height="13" ></a><map name="map1">
$imageMap
</map>
...
<a href="http://172.25.10.121/cgi-bin/isa_current_database_report.pl?data=sitesperhour&time=$time"><img src="http://172.25.10.121/excel.gif" border="0" align="top" width="16" height="13" ></a><map name="map2">
$imageMap2
</map>
