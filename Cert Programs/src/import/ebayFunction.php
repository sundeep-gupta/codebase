...
function DoEBAY() {
	...
	$technical = 211;
	$fees = array();
	$fees["Standard - $5,000"] = 50;
	$fees["Re-Certification - $2,500"] = 51;

	$statuses = array();
	$statuses["Testing Complete"] = 33;
	$statuses["Cancelled / Long-Term Hold"] = 34;
	$statuses["Registration"] = 32;

	//get all the certs in the table along with the ids of the users
	$sql = "SELECT java.contact.email, java.project_contact.contype, ebay.product.*
			FROM java.project_contact, java.contact, ebay.product
			WHERE ebay.product.pro_id=java.project_contact.project AND java.contact.con_id=java.project_contact.contact
	...
	for($x=0;$x<$certsCnt;$x+=2) {
		...
		//now add all the ebay logs for this entry
		$sql = "SELECT ebay.product_comment.* FROM ebay.product_comment WHERE product='".$certs[$x]["pro_id"]."' ORDER BY cr_date";
