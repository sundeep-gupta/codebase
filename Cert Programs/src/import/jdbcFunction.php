...
function DoJDBC() {
	...
	$statuses = array();
	$statuses["Registration"] = 39;
	$statuses["Testing Complete"] = 40;
	$statuses["Cancelled / Long-Term Hold"] = 41;

	//get all the certs in the table along with the ids of the users
	$sql = "SELECT java.contact.email, java.project_contact.contype, jdbc.product.*
			FROM java.project_contact, java.contact, jdbc.product
			WHERE jdbc.product.pro_id=java.project_contact.project AND java.contact.con_id=java.project_contact.contact
	...
	for($x=0;$x<$certsCnt;$x+=2) {
		...
		//now add all the jdbc logs for this entry
		$sql = "SELECT jdbc.product_comment.* FROM jdbc.product_comment WHERE product='".$certs[$x]["pro_id"]."' ORDER BY cr_date";
