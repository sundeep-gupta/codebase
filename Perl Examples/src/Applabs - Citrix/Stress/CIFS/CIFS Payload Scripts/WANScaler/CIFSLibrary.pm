...
sub cifs_read {
    ...
    print "Info of cifs_read\n";
    print "@_";
    print "\n".$drive_letter.":\\".$path."\\".$file_name;
...
sub copy_file {
	...
	#	print "Copying ". $source_path.$source_file_name." to ".$dest_path.$dest_file_name."\n";
...
sub copy_directory {
	my ($source_path,$source_dir,$dest_path,$dest_dir) = @_;
    $err_msg = 'Source_path not valid', return 0 unless $source_path;
    my $xcopy = "XCOPY $source_path $dest_path ";
    $xcopy = "COPY $source_path\\*.*  NUL: " unless $dest_path;
	my @timer1 = gettimeofday();
    if(-d $source_path) {
 #   	print "$source_path is a directory";
        opendir(DIR,$source_path);
        my @files = readdir(DIR);
        shift(@files);shift(@files);
        closedir(DIR);
        foreach my $file (@files) {
        	if(-e $source_path."\\".$file) {
#            	print "copying $file";
                my	$i_time = copy_file($source_path,$file);
