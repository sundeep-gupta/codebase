...
sub __compute_cpu {
    ...
    my $cpu = sprintf "%.3f",($usage_sum / ($usage_sum + $idle_sum))*100;
    return $cpu;
}

################# GET SEND COMPRESSION RATIO ##################################
sub get_send_compression_ratio{
	my ($self) = @_;

	my $param1 = "CompressionClearTextBytes";
	my $param2 = "CompressionCipherTextBytes";

	my $response1 = $self->get_system_variable($param1);
	my $response2 = $self->get_system_variable($param2);

    if( $response1
    and $response1->{$param1}
    and $response1->{$param1}->{'Fault'} ) {
    	return 0;
    }

	if( $response2
    and $response2->{$param2}
...
#    print Dumper($response1);
#    print Dumper($response2->{$param2});
#	print $response1->{"Total"},"Hi ",$response2->{"Total"},"\n";
	my $ratio = ($response2->{"Total"} != 0)? sprintf("%.2f", $response1->{"Total"}/$response2->{"Total"})
    										: "0";
    return ($ratio >=1) ? $ratio : 0;
}
