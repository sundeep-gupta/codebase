...
my ($drive_letter, $ip, $shareNo, $shareLast) = @ARGV;
#VARIABLE DECLARATIONS
my $outFile = "C${drive_letter}Time.txt";

my %write_list = ("C:\\smlab\\testcase3\\testdos\\Files\\10MBFile" => "${drive_letter}:\\Files\\",
				   "C:\\smlab\\testcase3\\testdos\\Files\\1MBFiles" => "${drive_letter}:\\Files\\",
			       "C:\\smlab\\testcase3\\testdos\\Files\\100KBFiles" => "${drive_letter}:\\Files\\",
				   "C:\\smlab\\testcase3\\testdos\\Files\\10KBFiles" => "${drive_letter}:\\Files\\");

my $networkPath = "\\\\10.0.3.$ip\\share$shareNo";

my $unmapFile = "unmap${drive_letter}.txt";
my $mapFile = "map${drive_letter}.txt";
for(;;){
	for (my $i = $shareNo; $i< $shareLast ; $i++) {
	
		print "Using $i share";
		sleep(3);
		if (unmapDrive($drive_letter.":", $unmapFile) && mapDrive($drive_letter.":", $networkPath, $mapFile)) {
			my ($key1, $value1);
			while (($key1,$value1) = each(%write_list)) {
...
my ( $src, $dst, $outFile) = @_;
	print `xcopy $src $dst /Z /E /Y >> $outFile 2>&1`;
	if( !($? == -1 || ( $? & 127 ) ) && ( -f $outFile)) {
