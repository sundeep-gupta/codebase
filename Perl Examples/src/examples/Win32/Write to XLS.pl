...
$excel_name = getDir()."Template.xls";
...
if (@ARGV==0) {
	...
	if (@ARGV[0] != 1 and @ARGV[0] != 2 and @ARGV[0] != 4 and @ARGV[0] != 8 ) {
...
my $Sheet = $book->Worksheets("SM Execution");
...
for ($i=0;$i<(@throughput/2);$i++) {
	...
	$Sheet->Cells(6,$colIndex)->{'Value'} = @throughput[$i];
	$Sheet->Cells(6,$colIndex-1)->{'Value'} = @throughput[$i+3];
	...
	$val = $size."/".$average[$i];
	$Sheet->Cells(20,$colIndex)->{'Value'} = $val;

	$val = $size."/".$average[$i+3];
	$Sheet->Cells(20,$colIndex-1)->{'Value'} = $val;
