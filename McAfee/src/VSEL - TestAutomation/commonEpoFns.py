...
def EPOAssignPolicy(strNode, strPolicy, strProduct="EPOAGENTMETA", 
    ...
    Returns: True for success, otherwise False
    '''
    logging.debug("Assigning policy " + strPolicy + " for " + strProduct + " to " + strNode)
    ...
    try:
        ...
        logging.error("Failed to assign " + strPolicy + " policy to " + \
                 strNode + ": " + str(inst))
        return False
    return True

if __name__ == '__main__' :
    EPODeleteAllTasks(gethostname())
    ...
    '''
    task = AgentUpdateTask("immediate update task")
    taskSched = EPOTaskSchedule(taskSched.SCHED_TYPE_RUN_IMMEDIATELY)
    taskSched.setStartDateTime(datetime.date.today().replace(day = datetime.date.today().day+3))
    pprint (EPOCreateAgentTask(gethostname(), task, taskSched))
    '''
    
    print "Now waking up.... lets try update "
    #EPOAgentWakeup(gethostname(), False)
