...
my $final_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/Application-Launch.log";
my $info;

open (FP,"$ENV{ROOT_PATH}/ssm_perf/AppList.txt") || warn "Couldn't open config file for Application Launch Performance Test\n";
my @readme = <FP>;
close (FP);
...
sub execute (){
    ...
    foreach my $testcase(@readme) {
        ...
        system (`osascript $ENV{ROOT_PATH}/ssm_perf/Includes/LaunchCloseApps.scpt "$testcase"`); 
        #get the end time
        $etime = time();
