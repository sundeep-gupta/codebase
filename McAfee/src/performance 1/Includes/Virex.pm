...
# sub-routine for starting ods in SSM
sub StartODS(){
        ...
        print "Running cp $VirexTaskdatabase /usr/local/McAfee/AntiMalware/var/";
        system (`rm /private/tmp/ScanFolder`) if (-e '/private/tmp/ScanFolder'); 
        system (`ln -s $ScanPath /private/tmp/ScanFolder`); 
        #print "Running ODS -- /usr/local/McAfee/AntiMalware/VShieldTaskManager $TaskID\n";
        print "Running /usr/local/McAfee/AntiMalware/VShieldTaskManager $TaskID";
        system (`/usr/local/McAfee/AntiMalware/VShieldTaskManager $TaskID`); 
        print "\nFinished ODS Task...\n\n";
}
...
sub OpenFiles(){
	...
	while (my $line = <TMP>)
	{
		...
		#	open (FILE, "$line") || warn "Couldn't open :$line: for writing $!: \n";
	        #	close (FILE);
                        system("cp $line /tmp/");
                #       system("rm /tmp/tmp_file") if (-e "/tmp/tmp_file");
		}
		$count++;
	}
	close(TMP);	
...
{
my($configvalue) = @_;
my $sourceconfigfile = new Config::Natural "$ENV{ROOT_PATH}/ssm_perf/Includes/PerfConfig.txt";
...
$date2month = $date2month - 1;
my $starttimesec = timelocal($time1sec,$time1min,$time1hr,$date1day,$date1month,2008);

my $endtimesec = timelocal($time2sec,$time2min,$time2hr,$date2day,$date2month,2008);
...
sub CreateFilesPSS() {
    ...
    my $clean_file = "This is a clean file";
    my $count = 10000;
    if ($type =~ /clean/) {
        my $tmp = "/tmp/test/clean";
        chomp($tmp);
        $count=0;
        for ($i=0;$i<100000;$i++) {
            if ($count == 10000) {
    ...
    if ($type =~ /mixed/) {
        ...
        for ($i=0;$i<100000;$i++) {
            ...
            open(FP,">$tmp.$i");
            #open(FP,">$tmp");
            print FP "X5O\!P\%\@AP\[4\\PZX54\(P\^\)7CC\)7\}\$EICAR\-STANDARD\-ANTIVIRUS\-TEST\-FILE\!\$H\+H\*";
            close (FP);
        }
    }      
...
my( $opt,$pref_opt) = @_;
print "$ENV{ROOT_PATH}/ssm_perf/PrefChanger -k $opt -v $pref_opt\n"; 
system("$ENV{ROOT_PATH}/ssm_perf/PrefChanger -k $opt -v $pref_opt");
