...
my @search;
my $check = `cat /Volumes/Data/ssm_perf/Reports/Boottime.log`;
if ($check =~ /LOGIN/) {
...
system("/Volumes/Data/ssm_perf/Reports/time_mod_rep") if (-e "/Volumes/Data/ssm_perf/Reports/time_mod_rep");
open (FP,">/Volumes/Data/ssm_perf/Reports/time_mod_rep");
...
print FP "$var";
foreach my $t (@search) {
my $val1 = `cat /Volumes/Data/ssm_perf/Reports/Boottime.log | grep '$t'`;
my @tmp1 = split('\n',$val1);
my $b;
foreach my $a (@tmp1) {
    chomp($a);
