...
#   MSC Readiness done by - Aravind Kamble                                                                                                       #
# Description:                                                                                             #
...
# MSC readyness is done by Aravind Kamble
...
#Check if copy samples script is running
my $copy_samples = `ps -ax -o command | grep copy_samples.sh | cut -c 22-37`;
system("echo $copy_samples > /tmp/temp.txt");
my $cc = `cat /tmp/temp.txt | cut -c 18-32`;

if($cc eq "copy_samples.sh")
{
	print "\n\n\t Copy Samples script is Running!.. Exiting !!\n\n";
	exit;
}

if(!-e "/tmp/copy_samples.txt")
{
	print "\n\n\t Copy samples script may not have copied the all samples, Please check for the file \/tmp\/copy_samples.txt \n\n";
	exit;
}
...
$datno = `defaults read /Library/Preferences/com.Mcafee.AntiMalware Update_DATVersion`;
my $datno_backup = $datno;

chomp ($configFileReader,$LVT_ROOT, $logPath, $collectionPath, $datno);

# Verify the data read from config file
if (!-e $logPath)
{
	print "\nLog Path $logPath does not exist!. Creating the $logPath .. \n";
	system("mkdir -p $logPath")
}
print "\nLog Path is $logPath\n";
if (!-e $collectionPath)
{
	print "\nCollection Path $collectionPath does not exist!.. Please check proper directory is entered in config file .. \n";
	exit;
}
print "\nCollection Path is $collectionPath\n";
if($logPath eq $collectionPath)
{
	print "\nPlease enter different paths for Log Path and Collection Path..\n";
	exit;
}

#my $collection_name = $configFileReader->value_of("\/"."$runType"."[*]");
#chomp $collection_name;

if (-e "/usr/local/McAfee/AntiMalware/$datno/mcscan.vlt")
{
...
# Set all ODS and OAS preferences
print "\nSetting the Preferences...\n";
system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"OAS_ScanArchive\" 1") == 0 or die "Error changing plist file, OAS Scan Archive\n";
system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"ODS_ScanArchive\" 1") == 0 or die "Error changing plist file, ODS Scan Archive\n";
system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"OAS_Exclusion_List\" '($collectionPath)'") == 0 or die "Error changing plist file, Adding Exclusions\n";
system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"ODS_Exclusion_List\" '($collectionPath)'") == 0 or die "Error changing plist file, Adding Exclusions\n";
...
foreach $runType (@$allRuns)
{
	...
	my $dircheck;
	my $dir_check = 1;
	foreach $dircheck (@$collections)
	{
		if (!-e "$collectionPath/$dircheck"){
			print "\nCollection $dircheck does not Exist!.. Please make sure proper directory name is entered in config file! \n";
			$dir_check = 0;
		}
	}
        exit if ($dir_check == 0);
	...
	foreach $collName (@$collections)
	{
		...
		{ 
			...
			# change the datno variable
			$datno=13;
			# Change the plist file to point to eit dat directory
			system("defaults write /Library/Preferences/com.Mcafee.AntiMalware \"Update_DATVersion\" $datno") == 0 or die "Error changing plist file\n";
			
			# Restart Scan Manager
			print "\nRestarting Scan Manager after changing the Preference!.. This might take some time, Please Wait ... \n";
			system("$LVT_ROOT/Includes/PrefChanger -k 1 -v 0") == 0 or die "Error: Failed to stop the services\n";
			sleep 60;
			system("$LVT_ROOT/Includes/PrefChanger -k 1 -v 1") == 0 or mumble(*FH, "Error", "Error: Failed to start the services\n");
			sleep 180;
			...
			#system("launchctl unload /Library/LaunchDaemons/com.mcafee.virusscan.ScanManager.plist") == 0 
			 system("$LVT_ROOT/Includes/PrefChanger -k 1 -v 0") == 0
 			or Virex_LVT::mumble(*LOG, "Error", "Error: Failed to stop Virex before copying the collection - $!\n");
...
#
		...
		{
			if($collName =~ /applemail/i)
			{
				#set OAS and ODS apple mail preferences to true
				system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"OAS_ScanMail\" 1") == 0 or die "Error changing plist file, OAS Mail\n";
				system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"ODS_ScanMail\" 1") == 0 or die "Error changing plist file, ODS Mail\n";		
		
				
			}
			else
			{
				#Set OAS and ODS apple mail preferences to false
				system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"OAS_ScanMail\" 0") == 0 or die "Error changing plist file, OAS Mail\n";
				system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"ODS_ScanMail\" 0") == 0 or die "Error changing plist file, ODS Mail\n";
			}
			
			# Re-initialize the scanners with non eit dat
			$datno = $datno_backup;
			
			# Change the plist file to point to eit dat directory
			system("defaults write /Library/Preferences/com.Mcafee.VirusScan \"Update_DATVersion\" $datno") == 0 or die "Error changing plist file\n";
