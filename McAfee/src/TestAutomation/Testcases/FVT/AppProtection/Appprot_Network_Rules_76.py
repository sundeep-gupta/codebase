...
# TestcaseID: 16658
# TestcaseDescription: Set Restricted Network Access for application: Scenario 76
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16658")
        logging.info("Description : Set Restricted Network Access for application: Scenario 76")
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self.rule) != CommonAppProFns.SUCCESS :
            ...
            return 1
        try :
            logging.debug("Checking the outgoing allow for TCP")
        ...
        try :
            logging.debug("Checking the outgoing allow with port for UDP")
    ...
    def _init_variables(self):
        ...
        #self.regex_allow_tcp = "Granted permission to %s for  creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.port, self.app_path)
        self.regex_allow_tcp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.port, self.app_path)
        self.regex_allow_udp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot_udp, self.host_ip, self.port, self.app_path)
