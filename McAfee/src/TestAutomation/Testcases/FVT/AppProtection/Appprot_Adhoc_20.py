...
# TestcaseID: 17443
# TestcaseDescription: Test AppPro functionality when application added to the AppPro rule gets changed using hard link
...
class TestCase(BaseTest):
	...
	logging.info("Initializing testcase %s" % testcaseName)
        ...
        self.rule1 = {"AppPath" : self.app_paths[0], "Enabled" : "1",
                     "ExecAllowed":"0",
                     "NwAction":"1"}

        self.rule2 = {"AppPath" : self.app_paths[1], "Enabled" : "1",
                     "ExecAllowed":"1",
                     "NwAction":"1"}
	...
	logging.info("Executing testcase %s" % testcaseName)
        ...
        # Step 3 : Allow Hardlink2 to execute
        if CommonAppProFns.addAppProtRule(self.rule2) != CommonAppProFns.SUCCESS :
            logging.error("Failed to set allow rule for %s" %self.rule2["AppPath"])
            return 1
        ...
        try :
            logging.debug("Moving %s to %s" % (self.link_names[0],
                                               self.link_names[1]))
            os.remove(self.link_names[1])
            logging.debug("Removed %s"% self.link_names[1])
	...
	logging.info("Performing cleanup for testcase %s" % testcaseName)
        ...
        # First hardlink has already been moved
            if os.path.exists(os.remove(self.link_names[1])) :
                os.remove(self.link_names[1])
        except :
