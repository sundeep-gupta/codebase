...
# TestcaseID: 16525
# TestcaseDescription: Testcase to verify expected functionality when network rules are created to allow TCP traffic.
...
class TestCase(BaseTest):
    ...
    def __init__(self):
        logging.info("TestcaseID : 16525")
        logging.info("Testcase to verify expected functionality when network rules are created to allow TCP traffic.")
    ...
    def init(self):
        ...
        _customRule=[]
        _tcpAllow = dict()
        _tcpAllow = {"IP":"ANY_IP","Port":"ANY_PORT","Protocol":"1",
                            "Direction":"3","Action":"1"}
        _udpDny = dict()
        _udpDny = {"IP":"ANY_IP","Port":"ANY_PORT","Protocol":"2",
                            "Direction":"3","Action":"2"}
        _customRule.append(_tcpAllow)
        _customRule.append(_udpDny)
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self._clientRule) != CommonAppProFns.SUCCESS:
            logging.error("Failed to add Rule for TCP Allow.")
            return 1
    ...
    def verify(self):
        try :
            ...
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace", _regex) == True:
                logging.info("TCP connection to server established.")
            else:
