...
# TestcaseID: 16527
# TestcaseDescription: Testcase to verify expected functionality when network rules are created to allow UDP traffic.0
...
class TestCase(BaseTest):
    ...
    def __init__(self):
        logging.info("TestcaseID : 16527")
        logging.info("Testcase to verify expected functionality when network rules are created to allow UDP traffic.")
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self._clientRule) != CommonAppProFns.SUCCESS:
            logging.error("Failed to add Rule for UDP Allow.")
            return 1
