...
# TestcaseID: 16615
# TestcaseDescription: Set Restricted Network Access for application: Scenario 33
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16615")
        logging.info("Set Restricted Network Access for application: Scenario 33.")
    ...
    def init(self):
        ...
        self.customRule=[]
        _cr = dict()
        _cr = {"IP":"ANY_IP","Port":"ANY_PORT","Protocol":"2",
    ...
    def verify(self):
        ...
        try :
            _p = subprocess.call([self.app_path_client,"UDP","127.0.0.1","5000","Message"], stdout=subprocess.PIPE)
            if _p == 0 :
                _regex="Granted permission to "+self.app_path_server+" for  accepting a UDP connection from 127.0.0.1:5000"
                time.sleep(10)
                ...
                logging.error("Failed to launch App")
                return 1

        except :
            logging.error("Failed to launch application")
            return 1
        logging.info("UDP Connection established and data been sent")
        return 0
