...
# TestcaseID: 16612
# TestcaseDescription: Set Restricted Network Access for application: Scenario 30
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16612")
        logging.info("Set Restricted Network Access for application: Scenario 30.")
    ...
    def init(self):
        ...
        _cr = dict()
        _cr = {"IP":self.hostIp,"Port":_portRangeStr,"Protocol":"2",
    ...
    def verify(self):
        ...
        try :
            ...
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("UDP connection not denied for port in the port range")
                return 1
        ...
        logging.info("Network access successfully denied for port in the port range")
        ...
        try :
            ...
            time.sleep(10)
            # For outside port range, the action should be allow since we added another sub-rule to allow all remaining...
            _regex="Granted permission to "+self.app_path+" for  creating a UDP connection to "+self.hostIp
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("UDP connection not granted for port outside the port range")
                return 1
        ...
        logging.info("Network access granted properly for port outside the port range")
        ...
        try :
            ...
            time.sleep(10)
            # For outside IP input values, the action should be allow since we added another sub-rule to allow all remaining...
            _regex="Granted permission to "+self.app_path+" for  creating a UDP connection to "+self.hostIp
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("UDP connection not granted for IP outside input values")
                return 1
        ...
        logging.info("Network access granted properly for IP address outside the IP input values")
