...
# TestcaseID: 16639
# TestcaseDescription: Set Restricted Network Access for application: Scenario 57
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16639")
        logging.info("Set Restricted Network Access for application: Scenario 57.")
    ...
    def init(self):
        ...
        self.randPortNum = random.randint(self.port_range_min, self.port_range_max)
        _cmd =  self.app_path_server +" "+self.hostIp+" "+ str(self.randPortNum)
    ...
    def verify(self):
        ...
        try :
            ...
            subprocess.call([self.app_path_client, self.hostIp, str(self.randPortNum),"Message"],\
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
            time.sleep(10)
            
            _regex="Granted permission to "+self.app_path_server+" for  accepting a UDP connection from "+self.hostIp
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("UDP connection not accepted for the given IP address")
                return 1
        ...
        logging.info("Network access successfully granted for given IP address")
