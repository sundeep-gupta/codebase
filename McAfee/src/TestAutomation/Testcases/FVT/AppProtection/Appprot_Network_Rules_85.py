...
# TestcaseID: 16667
# TestcaseDescription: Set Restricted Network Access for application: Scenario 85
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16667")
        logging.info("Description : Set Restricted Network Access for application: Scenario 85")
    ...
    def execute(self):
        ...
        try :
            logging.debug("Checking the outgoing deny with port %s for UDP" % self.udp_port_2)
    ...
    def _init_variables(self):
        ...
        self.regex_deny_tcp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.server_port_2, self.app_path)
        ...
        self.regex_deny_udp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot_udp, self.host_ip, self.udp_port_2, self.app_path)
