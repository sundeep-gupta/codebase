...
# TestcaseID: 16611
# TestcaseDescription: Set Restricted Network Access for application: Scenario 29
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16611")
        logging.info("Set Restricted Network Access for application: Scenario 29.")
    ...
    def verify(self):
        ...
        try :
            ...
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("TCP connection not denied for port in the port range")
                return 1
        ...
        try :
            ...
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("TCP connection not granted for port outside the port range")
                return 1
        ...
        try :
            ...
            _regex="Granted permission to "+self.app_path_client+" for  creating a TCP connection to "+self.hostIp
            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",_regex) != True:
                logging.error("TCP connection not granted for IP outside input values")
                return 1
