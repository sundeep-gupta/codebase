...
# TestcaseID: 16643
# TestcaseDescription: Set Restricted Network Access for application: Scenario 61
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16643")
        logging.info("Description : Set Restricted Network Access for application: Scenario 61")
    ...
    def execute(self):
        ...
        # Add rule to allow application execution for BOTH protocol 
    ...
    def verify(self):
        ...
        # Inside of range it must be allowed.
        logging.debug("Checking outgoing allow for TCP : %s" % self.regex_allow_tcp)
        ...
        logging.debug("Checking outgoing allow for UDP : %s" % self.regex_allow_udp)
    ...
    def _init_variables(self):
        ...
        self.rule     = {
                ...
                "NwAction"    : "3",
                "CustomRules" : [
                     # Add rule to allow outoging TCP|UDP connection in ANY PORT/IP
                     {
                        'IP'       : "ANY_IP",
                        'Port'     : "ANY_PORT",
                        'Protocol' : '3', # BOTH
        ...
        self.regex_allow_tcp = "Granted permission to %s for  creating a %s connection to %s:%s because  of profile rule match with %s"\
                % (self.app_path, self.prot, self.host_ip, self.server_port_1, self.app_path)
