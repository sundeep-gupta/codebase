...
# TestcaseID: 16646
# TestcaseDescription: Set Restricted Network Access for application: Scenario 64
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16646")
        logging.info("Description : Set Restricted Network Access for application: Scenario 64")
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self.rule) != CommonAppProFns.SUCCESS :
            ...
            return 1
        try :
            logging.debug("Checking the outgoing deny with port %s for TCP" % self.server_port_1)
    ...
    def verify(self):
        ...
        # Checking deny for TCP and UDP outgoing.
        logging.debug("Checking deny outgoing for TCP : %s" % self.regex_deny_tcp)
        ...
        logging.debug("Checking deny outgoing for UDP: %s" % self.regex_deny_udp)
    ...
    def _init_variables(self):
        ...
        self.regex_deny_tcp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s"\
                % (self.app_path, self.prot, self.host_ip, self.server_port_1, self.app_path)
        self.regex_deny_udp = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s"\
