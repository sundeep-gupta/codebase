...
def searchQuarantineMetaFile(regex):
    """Search given regex in Quarantine meta file"""
    time.sleep(10)
    return parseFile('/Quarantine/.Quarantine.meta', regex)
...
def getMountVolumeDetails(file , type):

    """
    Reading the contents of the xml file (for the specified type of mounting) 
    Returning the values back through a dictionary
    Using strip to strip off any additional whitespaces
    """
    doc = xml.dom.minidom.parse(file)
    
    details = {}
    node1 = doc.getElementsByTagName(type)
    node2 = node1[0].getElementsByTagName("username")
    details['username'] = node2[0].firstChild.data.strip()
    node2 = node1[0].getElementsByTagName("password")
    details['password'] = node2[0].firstChild.data.strip()
    node2 = node1[0].getElementsByTagName("ip")
    details['ip'] = node2[0].firstChild.data.strip()
    node2 = node1[0].getElementsByTagName("folder")
    if len(node2) > 0 :
        details['folder'] = node2[0].firstChild.data.strip()
    
    return details
   
# Function to return Cpu Usage by Scanners
def getCpuUsageByScanners():
    _cpuCmd="ps aux | grep VShield | grep -v grep | awk '{sum += $3} END {print sum}'"
    _retVal=subprocess.Popen(_cpuCmd,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    _OutPut=_retVal.communicate()[0].split('\n')
    return float(_OutPut[0])

# Function to return Cpu Usage
def getCpuUsage():
    _cucmd="ps aux | grep -v grep | awk '{sum +=$3} END {print sum}'"
    _retVal=subprocess.Popen(_cucmd,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    _output=_retVal.communicate()[0].split('\n')
    return float(_output[0])

def isMcAfeeAgentInstalled() :
    '''checks if mcafee Agent is installed or not'''
    return os.path.exists('/Library/McAfee/cma/bin/')

def installMcAfeeAgent(install_path) :
    ''' Runs installer script to install the mcafee agent'''
    logging.debug('Installer script path is ' + install_path)
    _cmd = [install_path, '-i']
    try :
        proc = subprocess.Popen( 'sh ' + install_path + ' -i ', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        proc.wait()
        (stdout, stderr) = proc.communicate()
        
        if proc.returncode == 0 :
            logging.debug("Successfully installed mcafee agent")
            logging.debug("Sleeping for 300 seconds")
            time.sleep(30)
            return True
        
    except :
        logging.error('Failed to run the installer script') 
        return False
