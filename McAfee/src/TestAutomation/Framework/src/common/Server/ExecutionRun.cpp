 ...
 void * ExecutionRun::executionThreadEntry(void * arg)
{
	...
	SVNInterface svnObj(pSelf->m_request.getSvnPath(), pSelf->getRunId());
	
	/** We need to checkout below folders for testcases execution:
		"Testplans" folder to get info about what testcases to be run
		"Testcases/Logs" folder to store the logs info during processing
		SuiteType:
			0/UnitTest - "Testcases/UnitTest" folder where unit tests are available
			1/BVT - "Testcases/FVT" folder where BVT testcases are available since BVT is subset of FVT
			2/FVT - "Testcases/FVT" folder where FVT testcases are available
			3/WhiteBox - "Testcases/WhiteBox" folder where Whitebox testcases are available
	 **/
	
	string testplansDir = TEST_PLANS_DIR;
	string tcLogsDir = string(TEST_CASES_DIR) + "/" + "Logs";
	string testcasesDir = string(TEST_CASES_DIR) + "/";
	
	if(pSelf->m_request.getSuiteType() == eUnitTest)
		testcasesDir += "UnitTest";
	else
	if(pSelf->m_request.getSuiteType() == eBVT)
		testcasesDir += "FVT";
	else
	if(pSelf->m_request.getSuiteType() == eFVT)
		testcasesDir += "FVT";
	else
	if(pSelf->m_request.getSuiteType() == eWhiteBox)
		testcasesDir += "WhiteBox";
		
	if(svnObj.performSvnCheckout(testplansDir) && svnObj.performSvnCheckout(tcLogsDir) && svnObj.performSvnCheckout(testcasesDir))
	{
		/** SVN checkout successful **/
		trace(serverTrace,1,"ExecutionRun::executionThreadEntry svn checkout successful ");
	}
	else
	{
...
void * ExecutionRun::executeNodeTestSequence(void * arg)
{
	...
	ExecutionNodeRun *nodeRun = new ExecutionNodeRun(hostName, nodeTestSequence, pSelf->getTestRequest().getTestURLPath(), pSelf->getSvnCheckOutPath(), pSelf->getRunId());
	if(!nodeRun)
		return NULL;
