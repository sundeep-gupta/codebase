...
sub DownloadEmails{
    ...
    my $OutlookApp=Win32::OLE->new("Outlook.Application",sub {$_[0]->Quit;});
	my $OutLookVersion=$OutlookApp->{Version};
    ...
    $Log->Message(2,*LOG,"Outlook version is $OutLookVersion");
    ...
    if($OS=~m/64/){
        ...
        Win32::Process::Create($Outlook,"C:\\Program Files (x86)\\Microsoft Office\\OFFICE11\\OUTLOOK.EXE","",0,NORMAL_PRIORITY_CLASS,".")if($OutLookVersion=~m/11/);
		Win32::Process::Create($Outlook,"C:\\Program Files (x86)\\Microsoft Office\\OFFICE12\\OUTLOOK.EXE","",0,NORMAL_PRIORITY_CLASS,".")if($OutLookVersion=~m/12/);
    }
    else{

        #system("C:\\Program Files\\Microsoft Office\\OFFICE12\\OUTLOOK.EXE");
		Win32::Process::Create($Outlook,"C:\\Program Files\\Microsoft Office\\OFFICE11\\OUTLOOK.EXE","",0,NORMAL_PRIORITY_CLASS,".")if($OutLookVersion=~m/11/);
		Win32::Process::Create($Outlook,"C:\\Program Files\\Microsoft Office\\OFFICE12\\OUTLOOK.EXE","",0,NORMAL_PRIORITY_CLASS,".")if($OutLookVersion=~m/12/);
	}
	sleep(5);
	my ($hwnd)=Win32::GuiTest::FindWindowLike(0,"Microsoft Outlook");
	Win32::GuiTest::SetForegroundWindow($hwnd);
	Win32::GuiTest::SendKeys("{F9}");
    ...
    $InboxItems=$OutlookInbox->{items}->{count}||0;
    while($InboxItems == 0){
    ...
    $EndTime=time;
    $OutlookApp->Quit();
	system("$Bin/Process.exe","-q","outlook");
...
#            $Log->Message(2,*LOG,"Deleted $MailsDeleted Emails on server..");
	...
	$Rpt->{"Iteration_".$CurrentIteration}->{"Send_".$Desc."_EndTime"}=localtime($EndTime);
	$Rpt->{"Iteration_".$CurrentIteration}->{"Send_".$Desc."_TimeTaken"}=$EndTime-$StartTime;
	$Rpt->write("$Bin/../Logs/Report.ini");
