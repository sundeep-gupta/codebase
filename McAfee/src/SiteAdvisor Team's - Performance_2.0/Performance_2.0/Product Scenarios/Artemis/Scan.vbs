...
If Unnamed.Length = 0 Then
	WScript.Echo "Scan [Directory for RightClick Scan] Scan Type(1=Manual Scan 2= Right Click Scan 4= Scheduled Scan 8=Restart Scan) ..."
	WScript.Quit(1)
End If

Scan_Type=CInt(WScript.Arguments(0))
If Scan_Type=1 Then
	WScript.Echo "Starting Manual Scan ....."
ElseIf Scan_Type=2 Then
	WScript.Echo "Starting RightClick Scan ....."
	WhatToScan=WScript.Arguments(1)
	If WhatToScan="" Then 
		WScript.Echo "Directory for RightClick Scan not specified"
		WScript.Quit
	End if
ElseIf Scan_Type=4 then
	WScript.Echo "Starting Scheduled Scan Scan ....."
End If
...
Dim MCODS_SCANTYPE_RIGHT_CLICK

MCODS_SCANTYPE_RIGHT_CLICK=Scan_Type
...
Set logg=fso.OpenTextFile("..\Logs\ODS.log",8,1)
...
Set fldrs=fso.GetFolder(WhatToScan)
...
If Err.Number <> 0 Then
	WScript.Echo "Error Occurred during Scan. Error Code : " & Err.Number
	logg.WriteLine "Error Occurred during Scan. Error Code : " & Err.Number
	WScript.Quit(1)
End if