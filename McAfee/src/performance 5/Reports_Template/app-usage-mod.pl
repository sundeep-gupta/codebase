...
system("rm $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep") if (-e "$ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep");
open (FP,">$ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep");
...
print FP "$var";
foreach my $t (@search) {
my $val1 = `cat $ENV{ROOT_PATH}/ssm_perf/Reports/Application-Usage.log | grep '$t'`;
my @tmp1 = split('\n',$val1);
my $b;
foreach my $a (@tmp1) {
    chomp($a);
...
if (-e "/usr/local/McAfee") {
    `cp $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_msc`;
}
if (-e "/usr/local/vscanx") {
    `cp $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_vsmac`;
}
if ( ! -e "/usr/local/McAfee" && ! -e "/usr/local/vscanx") {
    `cp $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage_mod_rep $ENV{ROOT_PATH}/ssm_perf/Reports/app_usage`;
}
