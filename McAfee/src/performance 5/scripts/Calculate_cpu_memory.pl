...
{ 
  ...
  if ( $arg[0] == 1 )
  { 
    $cpu_mem_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/Cpu_Mem_Install.log"; 
    $time_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/InstallTime.log";
    $avg=1;
  } 

  if ( $arg[0] == 2 )
  { 
    $cpu_mem_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/Cpu_Mem_Idle.log"; 
    $time_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/IdleTime.log";
   } 

  if ( $arg[0] == 3 )
  {
    $cpu_mem_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/Cpu_Mem_Update.log"; 
    $time_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/UpdateTime.log";
    $avg=3;
  } 

  if ( $arg[0] == 4 )
  { 
    $cpu_mem_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/Cpu_Mem_Scan.log"; 
    $time_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/ScanTime.log";
  } 
...
#Caculate Average Time
  ...
  if ($avg == 1 ) 
  {
    $read_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/InstallTime.log";
    open (FP,"$read_log"); 
    foreach $line (<FP>)
    { 
  ...
  if ($avg == 3 ) 
  { 
    $read_log = "$ENV{ROOT_PATH}/ssm_perf/Reports/UpdateTime.log";
    open (FP,"$read_log"); 
    foreach $line (<FP>)
    {
