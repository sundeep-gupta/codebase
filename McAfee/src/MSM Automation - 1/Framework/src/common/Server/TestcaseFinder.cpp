...
TestcaseFinderObj::TestcaseFinderObj(const string &svnCheckoutPath) 
{
	...
	trace(serverTrace,1,"TestcaseFinderObj::TestcaseFinderObj - TestpalnDir:%s",testPlansDir.c_str());
...
{
	trace(serverTrace,1,"TestcaseFinderObj::enumerateSvnDir- Invoked");
	try
	{
		/** Check if directory. */
				 ...
				 ++dir_itr )
			{
				...
				if(bfs::exists(currPath) && !bfs::symbolic_link_exists(currPath))
				{
					...
					else
					{
						...
						pathsMap.insert(make_pair(currPath.leaf(), currPath.string()));
					}
				}
			}
		}
		else
		{
			...
			pathsMap.insert(make_pair(svnPath.leaf(), svnPath.string()));
		}
	}
	catch ( const std::exception & ex )
	{
...
{
	trace(serverTrace,1,"TestcaseFinderObj::getFullPath- Invoked");
	string testPath("");
