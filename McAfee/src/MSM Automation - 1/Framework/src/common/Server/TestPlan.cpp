...
#define TP_REQUIRED_DIR "/Testplan/RequiredDirectory"
#include "trace.h"
	...
	m_testPlanName(testPlanName)
{
	...
	trace(serverTrace,1,"TestPlan::TestPlane PATH - %s:",m_planFullPath.c_str());
	try
	{
		...
		m_planRequiredDir = xmlObj.getXPathStringValue(TP_REQUIRED_DIR);
...
TestPlan::TestPlan(const TestPlan& rhs):
	...
	m_planRequiredDir(rhs.m_planRequiredDir)
{
	m_planSequence.clear();
...
void TestPlan::generatePlanSequence()
{
	...
	for(int i=0; i<m_testcaseObjs.size(); i++)
	{
		...
		if(!tcObj->getDepCheckComplete())
		{
			...
			{
				trace(serverTrace,1," TestPlan::generatePlanSequence - DependecyStr:%s:",dependsStr.c_str());
