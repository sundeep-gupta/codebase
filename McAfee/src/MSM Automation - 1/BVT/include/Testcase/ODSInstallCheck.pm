...
package Testcase::ODSInstallCheck;
use Testcase::ODS;
use Const;
use strict;
use File::Find;
...
sub verify {
	...
	$self->{'result'}  = {'ODSInstallCheck' => $PASS};
    $log->info("ODSInstallCheck is PASS");
    } else {
	$self->{'result'}  = {'ODSInstallCheck' => $FAIL};
    $log->info("ODSInstallCheck is FAIL");
    }
}
