...
sub verify {
    ...
    if ( $rh_config->{'write'} and $rh_config->{'write'}->{'file_count'} ) {
        $self->{'result'}->{'OASExclusion'} = $self->_verify_write($rh_config->{'write'}->{'file_count'},
							 $source_dir);
    } 
    if ( $rh_config->{'read'} and -e $source_dir) {
        $self->{'result'}->{'OASExclusion'} = $self->_verify_read( $source_dir) ;
    }
