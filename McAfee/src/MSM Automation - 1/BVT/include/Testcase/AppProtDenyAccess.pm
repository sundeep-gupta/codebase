...
package Testcase::AppProtDenyAccess;
use strict;
use Testcase;
use AppleScript;
...
sub execute {
    ...
    $self->{'result'}->{ 'AppProtDenyAccess'} = $PASS ;
}
...
sub verify {
    ...
    {
        $self->{'result'}->{ 'AppProtDenyAccess'} = $FAIL;
        return;
    }
	foreach my $app_name ( @$ra_applications) {
		...
		if( $self->is_application_excluded($ra_excl_app, $app_name) ) {
			$self->{'result'}->{'AppProtDenyAccess'} = $FAIL unless $self->is_application_launched($app_name) ;
			next;
		}
		
		if($product->get_other_app_pref() eq 'Deny') {
			$self->{'result'}->{'AppProtDenyAccess'} = $FAIL if $self->is_application_launched($app_name);
			next;
		}
	}
}
