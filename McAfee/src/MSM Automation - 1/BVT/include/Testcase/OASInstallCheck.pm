...
package Testcase::OASInstallCheck;
use Testcase::OAS;
use File::Find;
use Const;
use strict;
...
sub execute {
    ...
    $self->{'result'}   = { 'OASInstallCheck' => $PASS };
...
sub init {
    ...
    # Wait for all scanners to get loaded...
    sleep 180;
}
...
sub verify {
    ...
    if ( $rh_config->{'write'} and $rh_config->{'write'}->{'file_count'} ) {
        $self->{'result'}->{'OASInstallCheck'} = $self->_verify_write($rh_config->{'write'}->{'file_count'},
							 $source_dir);
    } 
    if ( $rh_config->{'read'} and -e $source_dir) {
        $self->{'result'}->{'OASInstallCheck'} = $self->_verify_read( $source_dir) ;
    }
