...
# TestcaseID: 16663
# TestcaseDescription: Set Restricted Network Access for application: Scenario 81
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16663")
        logging.info("Description : Set Restricted Network Access for application: Scenario 81")
    ...
    def init(self):
        ...
        self.cmd2     = [self.app_path, "UDP", self.host_ip2, self.port, "Test Message"]
        self.rule     = {
        ...
        self.regex2 = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip2, self.port, self.app_path)
    ...
    def execute(self):
        ...
        # See if app is trying to make network connection from any other Subnet. 
        # It should not be allowed
        try :
            logging.debug("Checking the outgoing allow with %s" % self.app_path)
            subprocess.call(self.cmd2, stdout=subprocess.PIPE)
        except :
            logging.debug("UDP client failed for outgoing connection as expected!")
            return 1
        return 0
    ...
    def verify(self):
        ...
        if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",self.regex) != True:
            ...
            return 1
        if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",self.regex2) != True:
            logging.error("Failed to match : %s" % self.regex2)
            return 1
        return 0
