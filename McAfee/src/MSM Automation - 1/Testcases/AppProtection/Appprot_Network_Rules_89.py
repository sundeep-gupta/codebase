...
# TestcaseID: 16671
# TestcaseDescription: Set Restricted Network Access for application: Scenario 89
...
class TestCase(BaseTest):
    ...
    def _init_variables(self):
        ...
        self.regex_deny = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.server_port_1, self.app_path)
        self.regex_allow = "Granted permission to %s for  creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.server_port_2, self.app_path)
