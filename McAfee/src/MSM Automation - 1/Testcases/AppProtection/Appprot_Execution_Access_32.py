...
# TestcaseID: 16472
# TestcaseDescription:  Verify that a binary of any application blocked doesn't allow the application to be launched 
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16472")
        logging.info("Description : Verify that a binary of any application blocked doesn't allow the application to be launched ") 

    def init(self):
        logging.info("Initializing testcase %s" % testcaseName)
        self.coca_app_path = os.path.dirname(os.path.abspath(sys.argv[0]))\
                           + "/data/SampleCocoaCarbonApps/Downloader.app"
        self.cocaBinaryPath = os.path.dirname(os.path.abspath(sys.argv[0]))\
    ...
    def execute(self):
        ...
        try:
            ...
            retval = subprocess.call(["open", self.coca_app_path])
            if(retval != 0): 
    ...
    def verify(self):
        ...
        if commonFns.isProcessRunning(self.appName) == True :
            logging.error("Error application :%s is running" % self.coca_app_path)
            return 1
        return 0
