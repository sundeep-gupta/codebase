...
# TestcaseID: 16661
# TestcaseDescription: Set Restricted Network Access for application: Scenario 79
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16661")
        logging.info("Description : Set Restricted Network Access for application: Scenario 79")
    ...
    def _init_variables(self):
        ...
        self.rule     = {
                ...
                "NwAction"    : "3",
                "CustomRules" : [
                     # Add rule to allow outoging connection
                     {
                        'IP'       : self.subnet,
                        'Port'     : self.port,
                        'Protocol' : '3', # BOTH
        ...
        self.regex_allow_udp = "Granted permission to %s for  creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot_udp, self.host_ip, self.port, self.app_path)
