...
# TestcaseID: 16568
# TestcaseDescription: Verify Unknwon applications can be set to prompt and the user is can deny the application to execute
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16568")
        logging.info("Verify Unknwon applications can be set to prompt and the user is can deny the application to execute .")
    ...
    def execute(self):
        ...
        if CommonAppProFns.alertDenyAlways("taf", "test") != True :
    ...
    def _launch_app(self):
        ...
        try :
            _p = subprocess.Popen([self.app_path,"UDP 127.0.0.1","5000","Message"], stdout=subprocess.PIPE)
            if _p == None :
    ...
    def verify(self):
        ...
        for key in self.rule:
            if self.rule[key]!=self.rule2[0][key]:
                ...
                return 1;
        # Lauching Application should fail because Deny all rule is applied
        try :
            logging.debug("Launching %s" % self.app_path)
            _retval=subprocess.call([self.app_path, "127.0.0.1", "5000","Message"],\
