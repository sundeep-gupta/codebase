    ...
    def execute(self):
        ...
        if ver == "6":
            ...
            try:
                ...
                return 1
            Cmd2= 'networksetup -setairportnetwork %s "Phoenix" "v1rexA1r"' % self.hardwarePort 
            try:
                retval = subprocess.call(Cmd1, shell=True)
                if retval != 0 :
                    logging.error("Unable to turn on the airport")
            ...
            if os.path.islink("/usr/sbin/airport") == True:
                ...
                try:
                    ...
                    return 1
            Cmd2 = "airport --associate=Phoenix --password=v1rexA1r"
            try:
                retval = subprocess.call(Cmd2, shell=True)
                if retval != 0 :
                    logging.error("Unable to make connection with Phoenix N/W")
                    return 1
            except:
                logging.error("Exception occured while making a connection with Phoenix N/W")
                return 1
            time.sleep(5)
