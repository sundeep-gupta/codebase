...
# TestcaseID: 16530
# TestcaseDescription: Verify an Application can be Allowed Network Access to all ports(unrestricted).
...
class TestCase(BaseTest):
    ...
    def __init__(self):
        logging.info("TestcaseID : 16530")
        logging.info("Verify an Application can be Allowed Network Access to all ports(unrestricted)")
    ...
    def verify(self):
        try :
            ...
            _regex="Granted permission to " + self._client + " for  creating a TCP connection to 127.0.0.1:6000"

            if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace", _regex) == True:
                logging.info("TCP connection to server established at port 6000 on server 127.0.0.1.")
            else:
