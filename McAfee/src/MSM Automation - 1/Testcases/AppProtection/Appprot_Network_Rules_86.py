...
# TestcaseID: 16668
# TestcaseDescription: Set Restricted Network Access for application: Scenario 86
...
class TestCase(BaseTest):
    def __init__(self):
        logging.info("TestcaseID : 16668")
        logging.info("Description : Set Restricted Network Access for application: Scenario 86")
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self.rule) != CommonAppProFns.SUCCESS :
            ...
            return 1
        try :
            logging.debug("Checking the outgoing allow with port %s" % self.server_port_1)
        ...
        try :
            logging.debug("Checking the outgoing deny with port %s" % self.server_port_2)
    ...
    def verify(self):
        ...
        logging.debug("Checking : %s" % self.regex_deny)
        if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",self.regex_deny) != True:
            logging.error("Failed to match : %s" % self.regex_deny)
            return 1

        # Outside of range it must be allowed.
        logging.debug("Checking : %s" % self.regex_allow)
        if commonFns.parseFile("/usr/local/McAfee/AppProtection/var/appProt.trace",self.regex_allow) != True:
            logging.error("Failed to match : %s" % self.regex_allow)
            return 1
        return 0
    ...
    def _init_variables(self):
        ...
        self.regex_deny = "Denied %s from creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.server_port_2, self.app_path)
        self.regex_allow = "Granted permission to %s for  creating a %s connection to %s:%s because  of profile rule match with %s" % (self.app_path, self.prot, self.host_ip, self.server_port_1, self.app_path)
