...
# TestcaseID: 16526
# TestcaseDescription: Testcase to verify expected functionality when network rules are created to block UDP traffic.
...
class TestCase(BaseTest):
    ...
    def __init__(self):
        logging.info("TestcaseID : 16526")
        logging.info("Testcase to verify expected functionality when network rules are created to block UDP traffic.")
    ...
    def execute(self):
        ...
        if CommonAppProFns.addAppProtRule(self._clientRule) != CommonAppProFns.SUCCESS:
            logging.error("Failed to add Rule for UDP deny.")
            return 1
