...
{
   double irisData[150][4] = {{5.1,3.5,1.4,0.2},
							{4.9,3.0,1.4,0.2},
{4.7,3.2,1.3,0.2},
{4.6,3.1,1.5,0.2},
{5.0,3.6,1.4,0.2},
...
/*	printf("cluster 1 size is : %d",cluster1Size);
	printf("\ncluster 2 size is : %d",cluster2Size);
	*/
for(i=0;i<4;i++){
cent1[i] = 0;
cent2[i] = 0;
...
for(i=0;i<cluster1Size;i++){
	cent1[0] = cent1[0] + cluster1[i][0];

	cent1[1] = cent1[1] + cluster1[i][1];

	cent1[2] = cent1[2] + cluster1[i][2];

	cent1[3]= cent1[3] + cluster1[i][3];
}
for(i=0;i<cluster2Size;i++){
	cent2[0] = cent2[0]+ cluster2[i][0];

	cent2[1]= cent2[1] + cluster2[i][1];

	cent2[2]= cent2[2] + cluster2[i][2];

	cent2[3]= cent2[3]+ cluster2[i][3];
}
if(cluster1Size != 0)
{
...
/*for(i=0;i<4;i++){
printf(" \n cent 1 :%lf, cent2 : %lf",cent1[i],cent2[i]);
}
getch();
*/
for(i=0;i<cluster1Size;i++){
...
/*printf("The Final Fitness from the Function is : %lf \n",finalFitness);*/
   return finalFitness;
}
...
int main()
{
  ...
  double norFinalFitness[NUMRUNS],finalFitnessVals[1000],temp[2][ROWS][COLS],finalPop[2][ROWS][COLS],temp1[2][COLS],new[2][COLS],temps[2][COLS],fitnessVals[ROWS],r,f=0.8;
 ...
 /* printf("\t %f",temp[d][i][j]);*/
  ...
  i = 0;
/*  for(d=0;d<2;d++){
  finalFitness = finalFitness+retFinFit(temp,COLS,i);
  }*/
  finalFitness = retFinFit(temp[0][i],temp[1][i],COLS);
/*  printf("\nfinal fitness is= %f",finalFitness);*/
...
/*	for(d=0;d<2;d++)
		{
		oldfit = oldfit + retFinFit(temp[d][i],COLS);
		}
		 printf("\n\nThe old fitness(temp) is %lf ", oldfit);*/
...
/*		  printf("\t %lf ", temp1[d][j]);*/
		...
		for(j=0;j<COLS;j++)
		{
		  r=(double) 1* rand()/ (RAND_MAX);
		  if(r<0.8)
			temps[d][j]=temp1[d][j];
		  else
			temps[d][j]=temp[d][i][j];
		}
			}
...
/*	for(d=0;d<2;d++){
		fitness = fitness+retFinFit(temps[d],COLS);
		}
		printf("\nThe fitness of the Resultant vector(temps) is %f \n", fitness);*/
fitness = retFinFit(temps[0],temps[1],COLS);
		...
		/*	printf("fitnessVals contains :%lf",fitnessVals[j]);*/
		}
...
/*		 printf("\n\n The Final Fitness is : %lf",finalFitness);*/
		 for(d=0;d<2;d++)
		{
		  for(j=0;j<ROWS;j++)
		  {
