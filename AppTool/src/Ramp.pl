...
my $size = $#computers + 1;
my $total_computer_count = $size;

# Fetch our configuration INI Ramp parameters
my $cfg = $IO->getConfigInfo();
my $ramp_count = $cfg->val( 'GLOBAL', 'RAMP_COUNT' );
my $ramp_time  = $cfg->val( 'GLOBAL', 'RAMP_TIME' );
...
print "Ramp Execution Start Time: \t $startTime \n";
...
RAMP: while ($elapsedTime <= $CONFIG->{'TEST_TIME_LENGTH'}) {
	
	# If we have no more computers to ramp up, then continue in a loop until our test time has completed
    if ($#computers == -1) {
        sleep 10;
        redo RAMP;
    }
    ...
    # Fetch our ramp list
    my @rampList = splice(@computers,0,$ramp_count);
    ...
    foreach my $computer (@rampList) {
        ...
        my $params = {
            "USER_COUNT" => $total_computer_count
        };
    ...
    print "\n[Status]: Completed sending request to $total_computer_count out of $size client machine(s).... continuing.... \n\n";
    ...
    print "\n[Status]: Completed receiving response $total_computer_count out of $size client machine(s).... continuing.... \n\n";
    ...
    last RAMP if ($elapsedTime > $CONFIG->{'TEST_TIME_LENGTH'});
    
    my $sleep_time = $ramp_time - $elapsed;
    sleep $sleep_time;
...
print "Ramp Execution End Time: \t $endTime \n";
print "Ramp Execution Elapsed Time: \t " . ($endTime - $startTime) . " secs. \n";
print "Ramp Execution 100\% Complete.\n\n";
...
=head1 Ramp.pl - Sends an XML request to a list of client machines and executes the 
given command on the machine.

=head1 SYNOPSIS

Ramp.pl --action=[action] --item=[item] --option=[option]
