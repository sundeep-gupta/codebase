...
class ShoppingCart {
    protected $contents, $total, $weight, $cartID, $content_type;
...
// insert current cart contents in database
	if (is_array($this->contents)) {
		...
		$dbConn->add_query('BB_CUSTOMER_BASKET',
				   "select products_id from " . TABLE_CUSTOMERS_BASKET . 
				   " where customers_id = '" . (int)$customer_id . "' and products_id = '" . 
				   tep_db_input($products_id) . "'");
		$product_query = $dbConn->execute('BB_CUSTOMER_BASKET',null);
		if ($product_query->numRows() <= 0) {
		    $dbConn->add_query('BB_CUSTOMER_BASKET_INSERT',
				       "insert into " . TABLE_CUSTOMERS_BASKET . " (customers_id, products_id, customers_basket_quantity,".
				                                                 " customers_basket_date_added) ".
				       "values ('" . (int)$customer_id . "', '" . tep_db_input($products_id) . "', '" . 
				                $qty . "', '" . date('Ymd') . "')");
		    $dbConn->execute('BB_CUSTOMER_BASKET_INSERT',null);
		    if (isset($this->contents[$products_id]['attributes'])) {
			reset($this->contents[$products_id]['attributes']);
			while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
			    $dbConn->add_query('BB_INSERT_BASKET_ATTR',
					       "insert into " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . 
					       " (customers_id, products_id, products_options_id, products_options_value_id) ".
					       "values ('" . (int)$customer_id . "', '" . tep_db_input($products_id) . "', '" . 
					                (int)$option . "', '" . (int)$value . "')");
			    $dbConn->execute('BB_INSERT_BASKET,ATTR',null);
			}
		    }
		} else {
		    $dbConn->add_query('BB_UPDATE_BASKET',
				       "update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . $qty . 
				       "' where customers_id = '" . (int)$customer_id . "' and ".
				       "products_id = '" . tep_db_input($products_id) . "'");
		    $dbConn->execute('BB_UPDATE_BASKET',null);
		}
	    }
	}
...
// reset per-session cart contents, but not the database contents
	...
	$dbConn->add_query('BB_BASKET_QUERY1',
			   "select products_id, customers_basket_quantity from " . TABLE_CUSTOMERS_BASKET . 
			   " where customers_id = '" . (int)$customer_id . "'");
	$products_query = $dbConn->execute('BB_BASKET_QUERY1',null);

	while ($products = $products_query->fetchRow(DB_FETCHMODE_ASSOC)) {
	    $this->contents[$products['products_id']] = array('qty' => $products['customers_basket_quantity']);
	    $dbConn->add_query('BB_BASKET_ATTR_QUERY1',
			       "select products_options_id, products_options_value_id from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . 
			       " where customers_id = '" . (int)$customer_id . "' and products_id = '" . 
			       tep_db_input($products['products_id']) . "'");
	    $attributes_query = $dbConn->execute('BB_BASKET_ATTR_QUERY1',null);

	    while ($attributes = $attributes_query->fetchRow(DB_FETCHMODE_ASSOC)) {
	...
	$this->content_type = false;
      ...
      if (tep_not_null($customer_id) && $reset_database == true) {
	  $dbConn->add_query('BB_DEL_BASKET', 
			     "delete from " . TABLE_CUSTOMERS_BASKET . " where customers_id = '" . (int)$customer_id . "'");
	  $dbConn->execute('BB_DEL_BASKET');
	  $dbConn->add_query('BB_DEL_BASKET_ATTR',
			     "delete from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " where customers_id = '" . (int)$customer_id . "'");
	  $dbConn->execute('BB_DEL_BASKET_ATTR');
      }
