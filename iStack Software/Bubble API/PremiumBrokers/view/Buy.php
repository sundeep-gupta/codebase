...
class BuyView extends HTMLPage {
    public function __construct($model) { 

	$index_page  = new BuyBody($model);
...
class BuyBody extends NoSideBarView {
    public function __construct($model) {
	$this->main_body = new Buy($model);
	parent::__construct( array('header' => array('active_menu' => 'Real Estate')), array('id' =>'wrap'));
    }
}


class Buy extends Div {
    protected $model;
	...
	$form = new RequestPropertyForm('controller.php?action=Buy&event=send_mail', 'Fill your details', $list, $form_type,
	    array('controller.php?action=Buy&event=show_commercial','controller.php?action=Buy&event=show_residential'));
