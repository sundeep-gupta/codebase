...
&fatal_error("$ml_txt{'223'}") if ($iamguest) ;

$MembersPerPage = $TopAmmount;
$maxbar         = 100;

$barmaxnumb = 500 if (!$barmaxnumb);
if ($barmaxdepend == 1) {
	$barmax = 1;
...
<table width="100%" cellpadding="3" cellspacing="0">
  <tr>
    <td align="left" width="100%"><span class="small"><b>
     <a href="$scripturl">$mbname</a> &rsaquo;
      $ml_txt{'331'}</b></span>
    </td>
  </tr>
</table>
...
$TableHeader .= qq(
<tr>
	<th $selUser onclick="location.href='$scripturl?action=ml;sort=username';" width="23%" align="center"
	    style="border: 1px; border-style: outset; cursor: pointer;"><b>$ml_txt{'35'}</b></th>
	<th width="4%" align="center"><img src="$imagesdir/email.gif" border="0" alt="$ml_txt{'307'}" /></th>
	<th width="4%" align="center"><img src="$imagesdir/www.gif" border="0" alt="$ml_txt{'96'}" /></th>
	<th $selPos onclick="location.href='$scripturl?action=ml;sort=position';" width="23%" align="center"
	    style="border: 1px; border-style: outset; cursor: pointer;"><b>$ml_txt{'87'}</b></th>
	<th $selPost onclick="location.href='$scripturl?action=ml;sort=posts';" width="23%" colspan="2" align="center"
	    style="border: 1px; border-style: outset; cursor: pointer;"><b>$ml_txt{'21'}</b></th>
	<th $selReg onclick="location.href='$scripturl?action=ml;sort=regdate';" width="23%" align="center"
	    style="border: 1px; border-style: outset; cursor: pointer;"><b>$ml_txt{'234'}</b></th>
</tr>
);

if ($LetterLinks ne "") {
	$TableHeader .= qq(<tr>
		<td colspan="7"><span class="small">$LetterLinks</span></td>
	</tr>
	);
}	

&MLTop      if ($FORM{'sortform'} eq "posts"    || $INFO{'sort'} eq "posts");
&MLDate     if ($FORM{'sortform'} eq "regdate"  || $INFO{'sort'} eq "regdate") ;
&MLPosition if ($FORM{'sortform'} eq "position" || $INFO{'sort'} eq "position"); 
&MLByLetter if ($INFO{'sort'} eq "" || $INFO{'sort'} eq "mlletter" || $INFO{'sort'} eq "username"); 
...
sub MLByLetter {
	...
	if ($memcount) {
		...
		if ($letter) { $yymain .= qq~<tr><td  colspan="7" align="center"><br /><b>$ml_txt{'760'}</b><br /><br /></td></tr>~; }
	}
	undef @ToShow;
	&buildPages(0);
...
sub showRows {
	...
	if ($user ne "") {
		...
		if (${$uid.$user}{'hidemail'} eq "checked" && !$iamadmin && $allow_hide_email eq 1) {
			$yymain .= qq~
			<td align="center" ><img src="$imagesdir/lockmail.gif" alt="Hidden Email" /></td>
		~;
		} else {
			$yymain .= qq~
			<td align="center" ><a href="$scripturl?action=mailto;user=$user" target="_blank"><img src="$imagesdir/email.gif" border="0" alt="${$uid.$user}{'email'}" /></a></td>
		~;
		}
		$yymain .= qq~
		<td align="center" >$wwwshow</td>
		<td width="1%">$memberinfo{$user}</td>
		<td width="20%"><center>${$uid.$user}{'postcount'}</center></td>
		<td width="1%">$Bar</td> 
		<td width="10%">$dr_regdate &nbsp;</td>
		</tr>~;
	}
}
...
<!-- 
	function SelDec(decparam, visel) {
		...
		for(i=vistart; i<=viend; i++) {
			if(visel == pagstart) pagedropindex += '<td  height="14" style="height: 14px; padding-left: 1px; padding-right: 1px; font-size: 9px; font-weight: bold;">' + i + '</td>';
		...
		if (vistart != viend) {
			if(visel == 'all') pagedropindex += '<td height="14" style="height: 14px; padding-left: 1px; padding-right: 1px; font-size: 9px; font-weight: normal;"><b>$pidtxt{"01"}</b></td>';
...
~;
	...
	sub buildPages {
		...
		if ($_[0]) {
			$yymain .= qq~
		<table width="100%" cellpadding="3" cellspacing="0" id="navigation">
		<tr>
		<th align="left" width="100%">
		<span ><a href="$scripturl" >$mbname</a> &rsaquo; $ml_txt{'331'} $numshow</span>
		</th>
		</tr>
		</table>
		<table id="board">
		...
		<div class="small" style="float: left; width: 50%; text-align: right;"><b>$ml_txt{'1'}</b> $SortJump</div>
		</th>
		</tr>
		</table>
		<table border="0" width="100%" cellspacing="1" cellpadding="3" id="board-head">
		$TableHeader
		~;
		} else {
			$yymain .= qq~
