...
#
# This Method is called when the YaBB.pl is called w/o any
# arguments... huh... good search
#
sub BoardIndex {
	my ($lspostid, $lspostbd, $lssub, $lsposttime,
	    $lsposter, $lsreply, $lsdatetime,
	    $lastthreadtime, @goodboards, @loadboards);
	...
	foreach $curentry (@entries) {
		...
		if (!$last_ip) {
		    $last_ip = qq~
				  ...
				  <span class="error">$boardindex_txt{'no_ip'}</span>
				  <span class="small"><i>~;
		}
		if ($name) {
			if (-e ("$memberdir/$name.vars")) {
				&LoadUser($name) unless (${$uid.$name}{'realname'});
				&LoadMiniUser($name);
				$numusers++;
				$users .= qq~$link{$name}~;
	...
	if ($INFO{'catselect'}) {
		...
		$curforumurl = qq~<a href="$scripturl" class="nav">$mbname</a> &rsaquo; $tmpcat~;
	}

	&Collapse_Load unless ($iamguest);
	
	# first get all the boards based on the categories found in forum.master
	foreach $catid (@categoryorder) {
		next if ($INFO{'catselect'} ne $catid && $INFO{'catselect'}) ; 
		$boardlist = $cat{$catid};
		...
		foreach $curboard (@bdlist) {
			...
			next if ($annboard eq $curboard && !$iamadmin && !$iamgmod) ;
	...
	foreach $curboard (@loadboards) {
		...
		foreach $curuser (split(/\,/, $bdmods)) {
			$iammodhere = 1 if ($username eq $curuser);
		}
	...
	foreach $catid (@categoryorder) {
		...
		if ($catcol{$catid} || $INFO{'catselect'} ne '' || $iamguest) {
			foreach my $boardinfo (@goodboards) {
				...
				# as we fill the vars based on all boards we need to skip any cat already shown before
				${$uid.$curboard}{'pic'} = "ann.gif"      if (${$uid.$curboard}{'ann'}  == 1) ; 
				 ${$uid.$curboard}{'pic'} = "recycle.gif" if (${$uid.$curboard}{'rbin'} == 1) ; 
				...
				foreach $curgroup (split(/\,/, $bdmodgroups)) {
					$iammod = 1 if (${$uid.$username}{'position'} eq $curgroup) ; 
	...
	if ($color && $noshow != 1) { $grpcolors .= qq~<div class="small" style="float: left; width: 49%;"><span style="color: $color;"> $title</span></div>~; }
