...
class googleSearchController extends JController
{
	...
	function save()
	{
		...
		$row =& JTable::getInstance('googleSearch', 'Table');
	...
	function listConfiguration()
	{
		...
		$db	=& JFactory::getDBO();
		$db->setQuery("SELECT * FROM #__googleSearch_conf");
		$rows = $db->loadObjectList();

		require_once(JPATH_COMPONENT.DS.'admin.googlesearch.html.php');
		HTML_googleSearch::listConfiguration($option, $rows[0]);
	}
}
