...
// Plugin Name changed to "Very Simple Image Gallery" - Version 1.5.2
...
class plgContentVsig extends JPlugin
{
	...
	function onPrepareContent(&$row, &$params, $limitstart) {
		...
		$thumb_subfolder = 'vsig_thumbs';
		$_ctrlfolder = '/images/stories/vsig_buttons/';
    ...
    JLoader::import( 'vsighelper', dirname( __FILE__ ).'/plugin_vsig' );
		...
		//plgContentVsigHelper::beCheckGD();
		...
		if (preg_match_all("#{vsig}(.*?){/vsig}#s", $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			...
			foreach ($matches[0] as $match) {
				...
				if ($dh = @opendir($mosConfig_absolute_path.$rootfolder.$_images_dir_)) {
					...
					$html2="<br />Very Simple Image Gallery:<br />No images found in folder ".$mosConfig_absolute_path.$rootfolder.$_images_dir_."<br />";
				}
				else {
					//you promised me a folder - where is it?
					$html2="<br />Very Simple Image Gallery:<br />Could not find folder ".$mosConfig_absolute_path.$rootfolder.$_images_dir_."<br />";
				}
				...
				if($noimage) {
					...
					//controls
					$_ctrl_fwd_ 		= (array_key_exists("cfwd",$vsig_overrides)&&$vsig_overrides['cfwd']!="")?($vsig_overrides['cfwd']):(trim($this->_params->get('ctrl_fwd', '&gt;&gt;')));
					$_ctrl_back_ 		= (array_key_exists("cbwd",$vsig_overrides)&&$vsig_overrides['cbwd']!="")?($vsig_overrides['cbwd']):(trim($this->_params->get('ctrl_back', '&lt;&lt;')));
					$_ctrl_height_ 	= 24;

					//check if images are used
					$_ctrl_fwd_type  = (is_file($mosConfig_absolute_path.$_ctrlfolder.$_ctrl_fwd_)&&((substr(strtolower($_ctrl_fwd_),-3)=='jpg')||(substr(strtolower($_ctrl_fwd_),-3)=='gif')||(substr(strtolower($_ctrl_fwd_),-3)=='png')))?("img"):("noimg");
					$_ctrl_back_type 	 = (is_file($mosConfig_absolute_path.$_ctrlfolder.$_ctrl_back_)&&((substr(strtolower($_ctrl_back_),-3)=='jpg')||(substr(strtolower($_ctrl_back_),-3)=='gif')||(substr(strtolower($_ctrl_back_),-3)=='png')))?("img"):("noimg");
					//calculations
					...
					$thumbdir=$mosConfig_absolute_path.$rootfolder.$_images_dir_.'/'.$thumb_subfolder.'/';
					...
					if($_usescript_){$vsig_script.="var vsig_".$identifier."= new Array();\n";}
					...
					for($a = 0;$a<$noimage;$a++) {
						if($images[$a]['filename'] != '') {
							...
							if($_cap_show_&&isset($$captions)){ //are captions activated and are there captions?
								if(array_key_exists($images[$a]['filename'],$$captions)){ //check for specific caption
									$cur_cap=array(${$captions}[$images[$a]['filename']][0],${$captions}[$images[$a]['filename']][1]);
									$cur_cap_set=1;
								}
							...
							if($_link_use_&&isset($$vsiglinks)){ //are links activated and are there links?
								if(array_key_exists($images[$a]['filename'],$$vsiglinks)){ //check for specific links
									$cur_link=${$vsiglinks}[$images[$a]['filename']];
								}
							...
							if(isset($$captions)){ //are there captions?
								if(array_key_exists($images[$a]['filename'],$$captions)){ //check for specific caption
									$cur_alt=${$captions}[$images[$a]['filename']][0];
								}
								elseif(array_key_exists("CAPDEFAULT",$$captions)){ //check for default caption
							...
							if($_link_use_&&isset($$vsiglinks)&&array_key_exists($images[$a]['filename'],$$vsiglinks)&&${$vsiglinks}[$images[$a]['filename']][1]!=""){
								$cur_alt=${$vsiglinks}[$images[$a]['filename']][1];
							...
							$cur_webpath=$mosConfig_live_site.$rootfolder.$_images_dir_."/".rawurlencode(utf8_encode($images[$a]['filename']));
							//create values for js
							$cur_webpath_js=$cur_webpath;
							...
							$thethumb = plgContentVsigHelper::beResizeImg($mosConfig_absolute_path.$rootfolder.$_images_dir_.'/'.$images[$a]['filename'],$thumb_subfolder,$_width_,$_height_,'keep','no',$_quality_);

							//prepare and encode webpath main image#######
							$cur_thumb_webpath=$mosConfig_live_site.$rootfolder.$_images_dir_."/".$thumb_subfolder.'/'.$thethumb[1];
							...
							$vsig_script.="'topimg".$identifier."',";			//0-topimg-id
							$vsig_script.="'".$cur_webpath_js."',";				//1-image-url
							$vsig_script.="'".$cur_cap_js[0]."',";				//2-alt
							$vsig_script.="'".$cur_cap_js[1]."',";				//3-captitle
							$vsig_script.="'".$cur_link_js[0]."',";				//4-captext
							$vsig_script.="'".$cur_link_js[1]."',";				//5-linkhref
							$vsig_script.="'".$cur_link_js[2]."',";				//6-linktext
							$vsig_script.="'".$cur_alt_js."',";						//7-linktarget
							$vsig_script.="'".$cur_thumb_webpath."');\n";	//8-thumb-url
							}
						}
					}
					...
					if($_sets_use_&&$_sets_number_>=2) {
						...
						if($_ctrl_back_type=="img"){$html4.="<img src='".$mosConfig_live_site.$_ctrlfolder.$_ctrl_back_."' alt='".substr($_ctrl_back_, 0, -4)."'/>";}
						...
						if($_ctrl_fwd_type=="img"){$html4.="<img src='".$mosConfig_live_site.$_ctrlfolder.$_ctrl_fwd_."' alt='".substr($_ctrl_fwd_, 0, -4)."'/>";}
...
/*
								if($_th_right_=="1"){ //thumbs are at the right -> seperate them from the main image
...
*/
					...
					if($_sets_use_&&$_sets_number_>=2) {
					//if thumbs are right PLUS sets are used => topimage-controls-ruler-thumbs
						if($_th_right_=="1"){
							$html2 .="<div class='vsig_ruler".$identifier." vsig_ruler'>\n";
							$html2 .=$html3;
							$html2 .="</div>\n";
					...
					//if thumbs are below PLUS sets are used => topimage-thumbs-controls
