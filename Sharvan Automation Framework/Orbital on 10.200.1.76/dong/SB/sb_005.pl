...
Test case = SB_4.3.2.1
Verify that accelerated TCP can co-exist with UDP traffic.
* The Pipe should be filled.
* The accelerated TCP should not crush UDP and vice versa.
* TCP sessions started first, then UDP sessions
...
is (run_iperf($host, "-t $duration -P $sessions ") , 1, "Softboost: Accelerated TCP and UDP in Inline Mode") ;
...
# The iperf server must also be running with the same options.
    ...
    print $OUTPUT "INVOKE UDP IPERF: $iperf_udp \n";
