...
Test Case = SB_4.3.1.5
Verify that the total sending rates are bound by the license limit

=head2 Test Category
Softboost

=head1 ENVIRONMENT
* 2 Orbs with softboost configured. Compression should be disable on the tested sessions.
* Lincence limit = 50 Mbsec.
* DR = 40Mbsec, 200RTT and 0% DROP (flow_1)
* DR = 30Mbps,  300RTT  (flow_2)
...
my $license_limit = 50;
my $expected_send_rate = 40; #The license limit is 50Mbps. The total BW available is 70Mbps.
...
#Verify the results ..
   ...
   print $OUTPUT "The expected sending rate: $expected_send_rate Mbits/sec\n";
   ...
   my $total_send_rate = $flow1_rate[$flow1_slot] + $flow2_rate[$flow2_slot];
   print $OUTPUT "The actual total sending rate  : $total_send_rate \n";

#The total send rate must be lesser than the license limit
   if ( ($license_limit  >= $total_send_rate) && ($flow1_rate[$flow1_slot + 1] eq "Mbits/sec" ) && ($flow2_rate[$flow2_slot+1]eq "Mbits/sec")) {
        print $OUTPUT "Seems to obey the license limit..OK \n";
        #now check if the pipe is filled
        if ($total_send_rate >= $expected_send_rate) { 
          print $OUTPUT "The pipe is verified filled..OK \n"; 
          return 1 } 
    } else { return  0 } #if neither the pipe is filled nor the limit is obeyed
}
sub config_DR
{
  print $OUTPUT "Configure the DR: $_[0]\n";
  my $set_bw = "ssh $_[0] \".\/set_bw.sh 30.30.20.0 40000 200;.\/set_bw.sh 30.30.40.0 30000 300;.\/set_bw.sh 20.20.0.0 30000 200\"";
  $set_bw = `$set_bw`;
