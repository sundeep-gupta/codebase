...
=head1 DESCRIPTION
Test Case = SB_4.3.2.2
This test case is very much the same as the softboost_003.pl, except:
1. This test is for Proxy mode. The client should be located on the Proxy Orb & communicate
to the remote server using the proxy IPs.
2. TCP accelerated sessions are created after UDP.
...
chomp ($host );
my $testcase="sb_006";
my $duration =50;                #min must be 50 => 30 sec for TCP
...
is (run_iperf($host, "-t $duration -P $sessions ") , 1, "Softboost: Accelerated TCP & UDP in Proxy Mode") ;
...
# This iperf UDP with -l500 -w 500 will use up 512 bytes/sessions
# The iperf server must also be running with the same options.
    $udp_host =~ s/20/30/;  #use the non-proxy IP for UDP test
    my $iperf_udp = "/usr/local/bin/iperf -c $udp_host $args -u -l 500 -w 500 > $udp_log &";
