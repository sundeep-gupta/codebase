...
=head1 DESCRIPTION
Verify that non-accelerated TCP can co-exist with acc TCP.
Non-acc TCP started first
...
my $host = $opt_s;
chomp ($host );
my $testcase="softboost_006";
my $duration =150;              #min must be 50 => 30 sec for TCP
my $sessions = 20;               #20 accelerated & 20 UDP sessions
my $sleep_time = $duration + 40; #running 20 acc & 20 non-acc against a server, need 50 extra.
