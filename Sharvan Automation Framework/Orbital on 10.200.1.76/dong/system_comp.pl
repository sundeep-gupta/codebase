...
# Bring up acc-TCP test    
    my $iperf_tcp = "/usr/local/bin/iperf -c $host -t $run_time -P 5 -F ./data/2.6x > $tcp_log &";
...
# while duration is not met
    ...
    my $minutes_left = $duration/60;
    print $OUTPUT "Time Left  $minutes_left minutes \n";
} end of while
