...
=head1 DESCRIPTION
Verify performance of one accelerated sessions over 
a range of predefined BWs
=head2 Test Category
Performance
...
getopt ("d, s, r") ;
if (  ! $opt_s ) {
  print "Usage: $testcase -d <dr> -r <RTT> -s <Iperf Server>\n" ;
  exit 1 ;
}
...
my $duration = 60;
my $sessions =1;
my $perc = 0.90;                 #percentage throughput expected - change this criteria if required.
my $expected_flow1_rate;
my $sleep_time = $duration + 20; #Tune this value, for test with large number of sessions.
my $OUTPUT = "STDOUT";
my $log_result = 0;        #saved output of the test?.
if ($log_result) {
...
#Start the test ...
foreach $bw (@bw_list) {
  $expected_flow1_rate = ($bw * $perc); #Should be in the range of 93%
  is (run_perf ($host, $bw, "-t $duration "), 1, "Performance: One session over a $bw Mbps") ;
  sleep 10;
  print $OUTPUT "-----------------------------------------------\n\n";
}
...
#   print "This is the logged result \n @flow1_rate \n";
   print $OUTPUT "Performance Test of $sess over $bw Mbps \n"; 
