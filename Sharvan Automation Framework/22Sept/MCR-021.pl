...
# 
# Script ID:			MCR-021
# Script Name:			MCR-021.pl
# Description:			Verifies integrity of mid-compressible-data transferred using CIFS from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-021.pl [ --stype <type of system, script is running on>
...
#Transfer file
if (($transfer_time = $global_library->copy_file("$working_folder$TYPE_OF_FILE/$file", $mapped_folder)) eq undef) {
	exit 2;
}
#Delete file from destination
if (!$global_library->delete_file("$mapped_folder$file")) {
	exit 2;
}
#Transfer file again
if (($transfer_time = $global_library->copy_file("$working_folder$TYPE_OF_FILE/$file", $mapped_folder)) eq undef) {
