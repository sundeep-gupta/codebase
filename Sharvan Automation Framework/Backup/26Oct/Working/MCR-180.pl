...
#
# Script ID:			MCR-180
# Script Name:			MCR-180.pl
# Description:			Verifies compression-ratio of non-compressible-data transferred using wget HTTP in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-180.pl [--size  <size of file>
...
my $url = "http://".$server."/http-path/$TYPE_OF_FILE/$file";
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	...
	#Transfer file
	if ((!$remote_library->wget_http($client, $url,"$share_path/$TYPE_OF_FILE/$file$ctr"))) {
		exit 2;
	}
