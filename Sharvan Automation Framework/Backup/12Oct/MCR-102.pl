...
# 
# Script ID:			MCR-102
# Script Name:			MCR-102.pl
# Description:			Verifies integrity of mid-compressible-data transferred using FTP in multi-session [different set of data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-102.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->get_ftp_using_threads($client, $server, $ftpusr, $ftppasswd,"/$TYPE_OF_FILE",$ref_files, "$share_path/$TYPE_OF_FILE", $ref_files))) {
	exit 2;
}
