...
# 
# Script ID:			MCR-059
# Script Name:			MCR-059.pl
# Description:			Verifies integrity of non-compressible-data transferred using CIFS in multi-session [same data] from client to server at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-059.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->copy_files_using_threads($client,"$share_path/$TYPE_OF_FILE",\@source_files, "//$server/$share/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
}
