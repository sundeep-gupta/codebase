...
# 
# Script ID:			MCR-071
# Script Name:			MCR-071.pl
# Description:			Verifies integrity of non-compressible-data transferred using wget ftp in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-071.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->wget_ftp_using_threads($client, $url, $ftpusr, $ftppasswd, "/$TYPE_OF_FILE",\@source_files,"$share_path/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
}
if ((!$remote_library->delete_files_using_threads($client,"$share_path/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
}
if ((!$remote_library->wget_ftp_using_threads($client, $url, $ftpusr, $ftppasswd, "/$TYPE_OF_FILE",\@source_files,"$share_path/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
