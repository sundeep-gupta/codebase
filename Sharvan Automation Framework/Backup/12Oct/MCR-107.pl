...
# 
# Script ID:			MCR-107
# Script Name:			MCR-107.pl
# Description:			Verifies integrity of non-compressible-data transferred using wget ftp in multi-session [different set of data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-107.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->wget_ftp_using_threads($client, $url, $ftpusr, $ftppasswd, "/$TYPE_OF_FILE",$ref_files,"$share_path/$TYPE_OF_FILE",$ref_files))) {
	exit 2;
}
