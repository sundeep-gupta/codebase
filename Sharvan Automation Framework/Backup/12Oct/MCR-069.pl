...
# 
# Script ID:			MCR-069
# Script Name:			MCR-069.pl
# Description:			Verifies integrity of non-compressible-data transferred using CIFS in multi-session [same data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-069.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->copy_files_using_threads($client,"//$server/$share/$TYPE_OF_FILE",\@source_files,"$share_path/$TYPE_OF_FILE",\@target_files))) {
	exit 2;
}
