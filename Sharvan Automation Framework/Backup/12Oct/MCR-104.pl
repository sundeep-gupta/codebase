...
# 
# Script ID:			MCR-104
# Script Name:			MCR-104.pl
# Description:			Verifies integrity of mid-compressible-data transferred using wget http in multi-session [different set of data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-104.pl [ --stype <type of system, script is running on>
...
#Transfer file
if ((!$remote_library->wget_http_using_threads($client, $url, "/$TYPE_OF_FILE",$ref_files,"$share_path/$TYPE_OF_FILE",$ref_files))) {
	exit 2;
}
