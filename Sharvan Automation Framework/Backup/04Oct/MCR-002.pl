...
# 
# Script ID:			MCR-002
# Script Name:			MCR-002.pl
# Description:			Verifies integrity of compressible-data transferred using CIFS from client to server at first level of compression
...
if ($help) {
	print "Usage: #perl mcr-002.pl [ --stype <type of system, script is running on>
...
#}
#Transfer file
if ((!$remote_library->ftp_put($client, $server, $ftpusr, $ftppasswd, "$share_path/$TYPE_OF_FILE/$file","/$TYPE_OF_FILE/$file"))) {
	exit 2;
}
