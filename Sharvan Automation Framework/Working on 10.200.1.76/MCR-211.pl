...
#
# Script ID:			MCR-211
# Script Name:			MCR-211.pl
# Description:			Verifies compression-ratio of mid-compressible-data transferred using wget FTP in multi-session [different set of data] from server to client at second level of compression
...
if ($help) {
	print "Usage: #perl mcr-211.pl [--size  <size of file>
...
my ($file, $ref_source_checksums, $target_checksum, $ctr, $ref_files, $first_pass, $url, $second_pass);
...
for ($ctr=0;$ctr<$CONNECTIONS;$ctr++) {
	$file=@{$ref_files}[$ctr];
	#Transfer file
	$url = "ftp://".$server."/ftp-path/$TYPE_OF_FILE/$file";
	if ((!$remote_library->wget_ftp($client, $url, $ftpusr, $ftppasswd,"$share_path/$TYPE_OF_FILE/$file"))) {
		exit 2;
	}
