...
sub Register {
    ...
    # if registration is disabled then its a fatal error.
    if ($regdisable && $iamguest) {
        &fatal_error("$register_txt{'3'}");
    }
    ...
    $tmpregname     = $FORM{'tusername'}    if ($FORM{'tusername'});
    $tmpregemail    = $FORM{'temail'}       if ($FORM{'temail'});
    $hidechecked    = qq~checked="checked"~ if ($FORM{'thideemail'}) ;
    $tmpregpasswrd1 = $FORM{'tpasswrd1'}    if ($FORM{'tpasswrd1'});
    $tmpregpasswrd2 = $FORM{'tpasswrd2'}    if ($FORM{'tpasswrd2'});
	...
	if (($filesanddirs ne '.')  &&
	    ...
	    $lngsel   = "";
	    $lngsel   = qq~selected="selected"~ if ($filesanddirs eq $language); 
    ...
    &validation_code                    if ($regcheck);
    &fatal_error("$register_txt{'2'}") 	if (!$iamguest); 
    $yymain .= qq~
<!-- Register Form -->    
<div style="padding: 0px;">
<table id="section" border="0" width="100%" cellspacing="0"  cellpadding="4" >
  <tr>
    <th  colspan="2" align="left">
	<img src="$imagesdir/register.gif" alt="$register_txt{'97'}" border="0" />
	<span>
	    <b>$register_txt{'97'}</b> $register_txt{'517'} 
	</span>
    </th>
  </tr>
~;
...
<tr>
    <td width="100%">
        * <b>$register_txt{'98'}:</b><br />
	<span class="small">$register_txt{'520'}</span>
    </td>
    <td>
    ...
    <td>
        * <b>$register_txt{'69'}:</b><br />
        <span class="small">$register_txt{'679'}</span>
    </td>
~;
...
~;
  ...
  <tr>
	<th width="100%" colspan="2" >
		<img src="$imagesdir/xx.gif" alt="$register_txt{'97'}" border="0" /> $register_txt{'764a'}
	</th>
  </tr>
  <tr>
	<td width="100%" colspan="2" >
...
~;
  ...
  <tr>
	<td width="100%" colspan="2">
		<center>
		<input type="submit" value="$register_txt{'97'}" /><br/><br/>
		</center>
	</td>
  </tr>
</table>
...
sub reg_banning {
    ...
    return 0 if ($username eq "admin" && $iamadmin);
    ...
    #
    # Parse each line of the file and if the given
    # user id , mail or IP is in the banned list then
    # add them to the ban log.
    #
	...
	if ($dummy eq "I") {
		...
		#
		# If the IP of the user is in banned list
		# Then add it to ban log, change user to guest & delete cookie.
		#
		...
		#
		# If this mail ID is banned, then do similar actions
		# as did for IP banning.
		#
		...
		#
		# If this user ID is banned, then do similar actions
		# as did for IP banning.
		#
...
sub Register2 {
    &fatal_error("$register_txt{'3'}") if ($regdisable && $iamguest) ;
    #
    # If user disagrees with agreement.
    # GO back to home page of Forum
    #
    ...
    #
    # remove special characters from the form elements
    #
    ...
    &fatal_error("Username prohibited by system") if ($hsize > 0);
    
    #
    # Validations & returning back if fails validation.
    #
    $member{'username'} = substr($member{'username'}, 0, 25)                       if (length($member{'username'}) > 25); 
    ...
    #
    # Validation from database (existing records)
    #
    $testname    = lc $member{'username'};
    $testemail   = lc $member{'email'};
    $is_existing = lc &MemberIndex("check_exist", "$testname");
    &fatal_error("($member{'username'}) $register_txt{'473'}")                   if ($is_existing eq $testname); 
    $is_existing = lc &MemberIndex("check_exist", "$testemail");
    &fatal_error("$register_txt{'730'} ($member{'email'}) $register_txt{'731'}") if ($is_existing eq $testemail); 

    &ToHTML($member{'email'});

    #
    # Not clear enough of this code
    # but looks like again some verifications happening here.
    #
    if ($regcheck) {
	...
	&fatal_error("$register_txt{'678'}$register_txt{'430'}!") if ($member{'email'} eq "$curban"); 
    }
	...
	if ($matchuser) {
	    if ($matchword) {
		&fatal_error("$register_txt{'244'} $reserved") if ($namecheck eq $reservecheck); 
   	    } else {
	        &fatal_error("$register_txt{'244'} $reserved") if ($namecheck =~ $reservecheck);
	    }
	}
    }
	...
	foreach $regline (@reglist) {
	    ...
	    ($dummy, $dummy, $regmember, $dummy) = split(/\|/, $regline);
   	    if ($reguser eq $regmember) {
		&fatal_error("$prereg_txt{'13'}");
		last;
	    }
	}
	...
	$yymain .= qq~
	<br /><br />
	<table border="0" width="600" cellspacing="1" id="section" align="center">
	<tr>
	<th colspan="2" >
	<img src="$imagesdir/register.gif" alt="$prereg_txt{'1a'}" border="0" /> <span class="text1"><b>$prereg_txt{'1a'}</b></span>
	</th>
	</tr><tr>
	...
	if ($emailpassword) {
		...
		<table id="section" width="300" cellspacing="1" align="center">
		<tr>
		<td>
		<img src="$imagesdir/register.gif" alt="$register_txt{'97'}" border="0" />
		<span class="text1"><b>$register_txt{'97'}</b></span></td>
		</tr><tr>
		<td align="center">
...
sub user_activation {
    ...
    &fatal_error("$prereg_txt{'14a'}") if (!-e "$memberdir/$reguser.pre" && -e "$memberdir/$reguser.vars") ;
    
    &fatal_error("$prereg_txt{'14'}") if (!-e "$memberdir/$reguser.pre"); 
