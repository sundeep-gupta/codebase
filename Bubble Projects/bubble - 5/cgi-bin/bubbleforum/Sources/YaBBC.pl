...
sub quotemsg {
	...
	if ($qauthor) {
		$qmessage =~ s~\/me\s+(.*?)(\n|\Z)(.*?)~<span style="color: #FF0000;">* $qauthor $1</span>$2$3~ig;
	}
...
sub DoUBBC {
	...
	$message =~ s~\[color=(.+?)\](.+?)\[/color\]~<span style="color:$1;">$2</span>~isg;
	...
	$message =~ s~\[font=(.+?)\](.+?)\[/font\]~<span style="font-family:$1;">$2</span>~isg;
