...
#
# three variables, username, password and $cookiesession are set with
# this method call
#
&LoadCookie;          # Load the user's cookie (or set to guest)

#
# sets iamguest variable if the username is Guest
# 
...
eval { &yymain; };
#
# Trap any errors in the main and print the corresponding
# error messages
#
&fatal_error("Untrapped Error:<br />$@") if ($@) ;
...
sub yymain {
	...
	if ($iamguest && $guestaccess == 0) {
		if (!(($action eq 'login') || ($action eq 'login2') ||
		      ($action eq 'register') || ($action eq 'register2') ||
		      ($action eq 'reminder') || ($action eq 'reminder2') ||
		      ($action eq 'validate') || ($action eq 'activate') ||
		      ($action eq 'resetpass')
		      )) {
			&KickGuest;
		}
	...
	if ($action ne "") {
		...
		# invoked with this url YaBB.pl?catselect=general
		require "$sourcedir/BoardIndex.pl";
		&BoardIndex;
	} else {
