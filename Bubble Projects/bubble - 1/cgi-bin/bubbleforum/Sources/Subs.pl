...
sub template {
	...
	if ($iamguest) {
		...
		if (!$regdisable) {
			$yymenu .= qq~$menusep<a href="$scripturl?action=register">$img{'register'}</a>~; }
	} else {
...
sub fatal_error {
	...
	&LoadIMs;
	$yymain .= qq~
<table border="0"  cellspacing="0" id="section-error" cellpadding="4">
  <tr align="left">
    <th colspan=3><span class="text1"><b>$maintxt{'106'}</b></span></th>
  </tr>
  <tr><td><br/></td></tr>
  <tr align="left">
    <td width="10"><br /></td>
    <td valign='center'><span class="text1">$e</span></td>
  </tr><tr align="center">
  <td colspan=3>
  <center>
  <a href="javascript:history.go(-1)">$maintxt{'193'}</a>
  </center>
  </td>
  </tr>
  <tr><td><br/></td></tr>
...
sub KickGuest {
	...
	$yymain .= qq~<div class="board" style="width: 400px; margin-bottom: 8px; margin-left: auto; margin-right: auto;">~;
	$yymain .= &sharedLogin;
	$yymain .= qq~</div>~;
	$yytitle = "$maintxt{'34'}";
	&template;
