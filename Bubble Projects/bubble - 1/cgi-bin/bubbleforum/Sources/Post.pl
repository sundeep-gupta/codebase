...
sub Post {
	&fatal_error($post_txt{'165'}) if ($iamguest && $enable_guestposting == 0); 
	&fatal_error($post_txt{'1'})   if ($currentboard eq ''); 
	my ($filetype_info, $filesize_info);
	my ($subtitle, $x, $mnum, $msub, $mname, $memail, $mdate, $mreplies,
	    $musername, $micon, $mstate, $msubject, $mattach, $mip, $mmessage,
	    $mns, $quotestart, $notify);
	my $quotemsg = $INFO{'quote'};
	$threadid = $INFO{'num'};

	($mnum, $msub, $mname, $memail, $mdate, $mreplies, $musername, $micon, $mstate) = split(/\|/, $yyThreadLine);
	&fatal_error($post_txt{'90'}) if ($mstate =~ /l/i); 
	&fatal_error($post_txt{'1'})  if ($mstate =~ /a/i && !$iamadmin && !$iamgmod); 
	...
	if    ($INFO{'title'} eq 'CreatePoll') {
		$pollthread = 1; $t_title = "$post_polltxt{'1a'}";
	} elsif ($INFO{'title'} eq 'AddPoll')    {
		$pollthread = 2; $t_title = "$post_polltxt{'2a'}";
	} elsif ($INFO{'title'} eq 'PostReply')  {
		$postthread = 2; $t_title = "$display_txt{'116'}";
	} else {
		$postthread = 1; $t_title = "$post_txt{'33'}";
	}

	&fatal_error($post_txt{'1'}) if ($pollthread == 2 && $useraddpoll == 0) ; 
	if ($postthread == 2 && $username ne "Guest") {
		...
		for ($i = 0; $i < @repliers; $i++) {
			...
			if ($repuser eq $username) {
				$tmprepliers[$j] = qq~$date|$repuser|1~; $isrep = 1;
			} else {
				$tmprepliers[$j] = qq~$reptime|$repuser|$isreplying~;
			}
			$j++;
		}
		if (!$isrep) {
	...
	$name_field = $realname eq ''
	  ? qq~      <tr>
    <td align="left" width="23%"><b>$post_txt{'68'}:</b></td>
    <td align="left" width="77%"><input type="text" name="name" size="25" value="$FORM{'name'}" maxlength="25" tabindex="2" /></td>
      </tr>~
	  : qq~~;

	$email_field = $realemail eq ''
	  ? qq~      <tr>
    <td width="23%"><b>$post_txt{'69'}:</b></td>
    <td width="77%"><input type="text" name="email" size="25" value="$FORM{'email'}" maxlength="40" tabindex="3" /></td>
      </tr>~
	  : qq~~;
...
sub Postpage {
	...
	if ($post eq "imsend") {
      ...
      <tr>
	<td width="1%"></td>
        <td width="22%"><b>$post_txt{'150'}</b></td>
        <td width="77%">
	...
	<span class="small">$post_txt{'748'}</span></td>
      ...
      <tr>
        ...
        <td width="22%"><b>$post_txt{'71'}:</b></td>
        <td width="77%" >
	...
	if ($pollthread == 2) {
		...
		if (!$enable_notification || $iamguest) {
			...
			if ($post ne "imsend") {
				$notification = qq~
	      <tr>
	        <td width="22%"><b>$post_txt{'131'}:</b></td>
...
//-->
    ...
    <table id="board" style="table-layout: fixed;">
      <tr>
        <th height="18"><span class="text1"><b>$yytitle</b></span></th>
      </tr>
~;
	...
	if ($pollthread) {
		...
		$yymain .= qq~
		<tr>
                <td width="23%" ><b>$post_polltxt{'6'}:</b> &nbsp;</td>
                <td width="77%">
		...
		for (my $i = 1; $i <= $numpolloptions; $i++) {
			$yymain .= qq~
                    <tr>
                      <td width="23%" > &nbsp; $post_polltxt{'7'} $i: &nbsp;</td>
                      <td width="77%"><input type="text" size="35" maxlength="$maxpo" name="option$i" value="$options[$i]" /></td>
                    </tr>~;
		}
		...
		$yymain .= qq~
		<tr>
                <td valign=top width="23%"><b>$post_polltxt{'59'}:</b></td>
                <td  width="77%"><textarea name="poll_comment" rows="3" cols="60" wrap="soft" onkeyup="if (document.postmodify.poll_comment.value.length > $maxpc) {document.postmodify.poll_comment.value = document.postmodify.poll_comment.value.substring(0,$maxpc)}">$poll_comment</textarea></td>
		</tr>~;
		}

		$yymain .= qq~
		<tr>
                <td width="23%" ><b>$post_polltxt{'32'}</b></td>
                <td width="77%" ><input type="checkbox" name="guest_vote" value="1"$gvchecked /> <span  class="small">$post_polltxt{'54'}</span></td>
              </tr><tr>
                <td width="23%" ><b>$post_polltxt{'26'}</b></td>
                <td width="77%"><input type="checkbox" name="hide_results" value="1"$hrchecked /> <span  class="small">$post_polltxt{'55'}</span></td>
              </tr><tr>
                <td width="23%"><b>$post_polltxt{'58'}</b></td>
                <td width="77%" ><input type="checkbox" name="multi_choice" value="1"$mcchecked /> <span  class="small">$post_polltxt{'56'}</span></td>
              </tr><tr>
                <td width="23%"><b>$post_polltxt{'60'}</b></td>
                <td width="77%"><input type="text" size="6" name="vote_limit" value="$vote_limit" /> <span  class="small">$post_polltxt{'61'}</span></td>
              </tr>
~;
	}
	...
	if ($pollthread != 2 || ($pollthread == 2 && $iamguest)) {
		if ($pollthread == 2) { $extra = ""; }
			...
			$yymain .= qq~
		      <tr>
		        <td align="left" width="23%"><b>$post_txt{'70'}:</b></td>
		        <td align="left"  width="77%"><input type="text" name="subject" value="$sub" size="50" maxlength="50" tabindex="1" onchange="updatTopic()" /></td>
		      </tr>
			~;
		}
		$yymain .= qq~
	...
	if ($pollthread != 2) {
		...
		if ($currentboard ne $annboard && $post ne "imsend" && (($iamadmin || $iamgmod || $iammod) && $sessionvalid == 1)) {
			$yymain .= qq~
		<tr>
			<td valign="top" width="23%"><b>$post_txt{'34'}:</b></td>
	...
	if ($enable_ubbc && $showyabbcbutt) {
		...
		$yymain .= qq~
  <tr>
      <td  width="23%"><b>$post_txt{'252'}:</b></td>
        <td valign="middle" width="77%">
...
~;
	...
	$yymain .= qq~
      <tr>
        <td width="23%"><b>$post_txt{'297'}:</b></td>
...
~;
	...
	$yymain .= qq~
        ...
        <td valign="top" width="23%"><b>$post_txt{'72'}:</b>
        <br /><br />
	~;
	...
	smiliecode = new Array($smilie_code_array)
        ...
        if((navigator.appName == "Netscape" && navigator.appVersion.charAt(0) >= 4) || (navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.charAt(0) >= 4) || (navigator.appName == "Opera" && navigator.appVersion.charAt(0) >= 4)) {
          ...
          document.write('<table id="board" height="120" width="120" border="0" cellpadding="2" cellspacing="1" align="center"><tr>');
          document.write('<th height="15" align="center" valign="middle" class="titlebg"><span class="small"><b>$post_smiltxt{'1'}</b></span></th>');
...
$notification
      <tr>
        <td width="23%"><b>$post_txt{'276'}:</b><br /><br /></td>
        <td width="77%"><input type="checkbox" name="ns" value="NS"$nscheck /> <span class="small"> $post_txt{'277'}</span><br /><br /></td>
      </tr>
$lastmod
~;
...
#	$message =~ s~(\w+://[^<>\s\n\"\]\[]+)\n([^<>\s\n\"\]\[]+)~$1$2~g;
    ...
    <tr>
      ...
      <td width="77%" ><input type="checkbox" name="notify" value="x"$notify /> <span  class="small">$post_txt{'750'}</span></td>
    </tr>
~;
		$destination = 'post2';
		$submittxt   = $post_txt{'105'};
...
<div class="bordercolor" style="padding: 1px; width: 100%; margin-left: auto; margin-right: auto;">
<table border="0" width="100%" cellpadding="3" cellspacing="0" id="board-head" style="table-layout: fixed;">
  <tr>
    <th>
...
<table id="board-head" style="table-layout: fixed;">
	<tr><th colspan="2"><div align='left'><span class="text1">
~;
...
<tr><th>
<span class="small">$post_txt{'279'}: $displaynamelink</span></th>
<th>
<span class="small">$post_txt{'280'}: $tempdate</span></th>
</tr>
<tr><td colspan="2">
