...
sub plushSearch1 {
	...
	$searchpageurl = $curposlinks ? qq~<a href="$scripturl?action=search" class="nav">$search_txt{'182'}</a>~ : $search_txt{'182'};
	$yymain .= qq~
...
<form class='search' action="$scripturl?action=search2" method="post" name="searchform" onsubmit="return submitproc()">
<table id="board" >
  <tr>
    <th colspan="3">
	<div width="100%" style="background: #99CC33;">
		<img src="$imagesdir/search.gif" alt="" />
		<span ><b>$search_txt{'183'}</b></span>
	</div>		
    </th>
  </tr><tr>
	<td width="05%">&nbsp;</td>
        <td width="45%"><b>$search_txt{'582'}:</b></td>
        <td width="50%">
	...
	<td width="05%">&nbsp;</td>
        <td valign="top"><b>$search_txt{'189'}:</b><br />$search_txt{'190'}</td>
        <td  >~;
	$allselected = 0;
	$isselected  = 0;
	$boardscheck = "";
...
sub plushSearch2 {
	...
	&fatal_error($search_txt{'337'}) if ($maxage  =~ /\D/)  ; 
	&fatal_error($search_txt{'337'}) if ($display =~ /\D/) ; 

	# restrict flooding using form abuse
	&fatal_error($floodtxt{'7'}) if ($display > $maxsearchdisplay) ; 

	my $userkind = $FORM{'userkind'};
	my $userspec = $FORM{'userspec'};
	if    ($userkind eq 'starter')    {
		$userkind = 1;
	} elsif ($userkind eq 'poster')     {
		$userkind = 2;
	} elsif ($userkind eq 'noguests')   {
		$userkind = 3;
	} elsif ($userkind eq 'onlyguests') {
		$userkind = 4;
	} else {
		$userkind = 0; $userspec = '';
	}
	&fatal_error($search_txt{'224'}) if ($userspec =~ m~/~)  ; 
	&fatal_error($search_txt{'225'}) if ($userspec =~ m~\\~)  ; 
	$userspec =~ s/\A\s+//;
	$userspec =~ s/\s+\Z//;
	...
	if    ($searchtype eq 'anywords')  {
		$searchtype = 2;
	} elsif ($searchtype eq 'asphrase')  {
		$searchtype = 3;
	} elsif ($searchtype eq 'aspartial') {
		$searchtype = 4;
	} else {
		$searchtype = 1;
	}
	&fatal_error($search_txt{'754'}) if ($search eq "" || $search eq " ") ; 
	&fatal_error($search_txt{'397'}) if ($search =~ m~/~)  ; 
	&fatal_error($search_txt{'397'}) if ($search =~ m~\\~) ;
	...
	threadcheck:
		foreach $curthread (@threads) {
		  ...
		  postcheck:
			for ($msgnum = @messages; $msgnum >= 0; $msgnum--) {
	...
	if (@messages) {
		...
		$yymain .= qq~<table id='board' cellspacing='0'/>
				<tr><td>&nbsp</td></tr>
				<tr>
				<td><center>$search_txt{'170'}</center>
				</td></tr>
...
<table id="board" style="table-layout: fixed;">
  <tr>
    ...
    <td width="95%">
	$catname{$board} / $boardname{$board} / <a href="$scripturl?num=$tnum/$msgnum#$msgnum"><span class="text1"><u>$msub</u></span></a><br />
	<span class="small">$search_txt{'30'}: $mdate</span>
    </td>
  </tr><tr>
    <td colspan="2" >
	<span>$search_txt{'109'} $tname | $search_txt{'105'} $search_txt{'525'} $mname</span>
    </td>
  </tr><tr>
    <td height="80" colspan="2"valign="top">$message</td>
  </tr><tr>
    <td colspan="2" >&nbsp;
~;
		if ($tstate != 1) {
...
~;
	...
	<table id='board'>
		...
		<center>
		<a href="$scripturl">$search_txt{'236'}</a> $search_txt{'237'}
		</center>
		</td></tr><tr><td>
		<cener>
