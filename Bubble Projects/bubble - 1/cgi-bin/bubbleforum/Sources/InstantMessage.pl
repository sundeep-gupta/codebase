...
<table id="board-head" style="table-layout: fixed;">
<tr>
	<td width="140">
		<span>&nbsp;<b>$rectext</b></span>
	</td>
	<td >
		<span><b>$inmes_txt{'118'}</b></span>
	</td>
</tr>
...
sub IMShow {
	...
	if ($INFO{'caller'} == 1) {
		$linktext = qq~<a href="$scripturl?action=im" >$inmes_txt{'316'}</a>~;
		$callvar  = "1";
		$rectext  = "$inmes_txt{'318'}";
		fopen(FILE, "$memberdir/$username.msg");
	} elsif ($INFO{'caller'} == 2) {
		$linktext = qq~<a href="$scripturl?action=imoutbox" >$inmes_txt{'320'}</a>~;
		$callvar  = "2";
		$rectext  = "$inmes_txt{'324'}";
		fopen(FILE, "$memberdir/$username.outbox");
	} elsif ($INFO{'caller'} == 3) {
		$linktext = qq~<a href="$scripturl?action=imstorage" >$inmes_imtxt{'46'}</a>~;
		$callvar  = "3";
...
<table id="board-head"> 
        <tr> 
          <th valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=im">$inmes_txt{316}</a></b></span></th> 
          <th valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imoutbox">$inmes_txt{320}</a></b></span></th> 
          <th valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imstorage">$inmes_imtxt{46}</a></b></span></th> 
          <th valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imsend">$inmes_txt{148}</a></b></span></th> 
          <th valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=profileCheck;username=$username">$inmes_txt{765}</a></b></span></th>
	  </tr> 
</table><br />~;
...
sub IMIndex {
	&fatal_error($inmes_txt{147}) if ($iamguest); 
	if ($action eq "im") {
		$yymain .= qq~ <table id="board-head"> 
        <tr> 
          <th valign="bottom" width="16%"><span class="small"><b><a href="$scripturl?action=im">$inmes_txt{316}</a></b></span></th> 
          <th valign="bottom" width="16%"><span class="small"><b><a href="$scripturl?action=imoutbox">$inmes_txt{320}</a></b></span></th> 
          <th valign="bottom" width="16%"><span class="small"><b><a href="$scripturl?action=imstorage">$inmes_imtxt{46}</a></b></span></th> 
          <th valign="bottom" width="16%"><span class="small"><b><a href="$scripturl?action=imsend">$inmes_txt{148}</a></b></span></th> 
          <th valign="bottom" width="16%"><span class="small"><b><a href="$scripturl?action=markims">$inmes_txt{764}</a></b></span></th> 
          <th valign="bottom"  width="16%"><span class="small"><b><a href="$scripturl?action=profileCheck;username=$username">$inmes_txt{765}</a></b></span></th>
        </tr> 
      </table><br />
      ~;
		...
		$movebutton  = qq~<td ><input type="submit" name="imaction" value="$inmes_imtxt{'50'}" /></td>~;
		fopen(FILE, "$memberdir/$username.msg");
...
<table id="board">
  ...
  <tr>
    <td width="03%" align="center" height="21">&nbsp;<b>#</b></td>
    <td width="25%">&nbsp;<b>$inmes_txt{'317'}</b></td>
    <td width="15%"><b>$senderinfo</b></td>
    <td width="35%"><b>$inmes_txt{'70'}</b></td>
    <td width="07%" align="center"><b>$status</b></td>
    <td width="05%">&nbsp;</td>
  </tr>
~;
	unless (@dimmessages) {
		$yymain .= qq~
  <tr>
    <td colspan="6" height="21">$inmes_txt{'151'}</td>
  </tr>
~;
	}
	$acount++;
  ...
  boardcheck: for ($counter = 0; $counter < @dimmessages; $counter++) {
		...
		$yymain .= qq~
  <tr>
    <td align="center" height="21">$innum</td>
    <td>$mydate</td>
    <td>$senderinfo2$musername</td>
    <td><a href="$scripturl?action=imshow;caller=$callerid;id=$messageid">$msub</a></td>
    <td>
~;
		if ($action ne "imstorage") {
			if    ($imnew == 1 && $action eq "im") {
				$yymain .= qq~<a href="$scripturl?action=imshow;caller=$callerid;id=$messageid"><img src="$imagesdir/imclose.gif" border="0" alt="$inmes_imtxt{'07'}" /></a>~;
			} elsif ($imnew == 2 && $action eq "im") {
				$yymain .= qq~<a href="$scripturl?action=imshow;caller=$callerid;id=$messageid"><img src="$imagesdir/answered.gif" border="0" alt="$inmes_imtxt{'08'}" /></a>~;
			} elsif ($imnew eq "" && $action eq "im") {
				$yymain .= qq~<a href="$scripturl?action=imshow;caller=$callerid;id=$messageid"><img src="$imagesdir/imopen.gif" border="0" alt="$inmes_imtxt{'09'}" /></a>~;
			} elsif ($imnew == 1 && $action eq "imoutbox") {
				$yymain .= qq~<a href="$scripturl?action=imshow;caller=$callerid;id=$messageid"><img src="$imagesdir/imopen.gif" border="0" alt="$inmes_imtxt{'21'}" /></a>~;
			} elsif ($imnew eq "" && $action eq "imoutbox") {
				$yymain .= qq~<img src="$imagesdir/imclose.gif" border="0" alt="$inmes_imtxt{'22'}" /><br /><span class="small"><a href="$scripturl?action=imcb;rid=$messageid;receiver=$rname" onclick="return confirm('$inmes_imtxt{'73'}')">$inmes_imtxt{'83'}</a></span>~;
			}
		}
		undef $quotecount;
		undef $codecount;
		...
		if ($action eq "im") {
			$quotemenu = qq~<a href="$scripturl?action=imsend;caller=$callerid;num=$counter;quote=1;to=$rname;id=$messageid">$inmes_txt{'145'}</a>$sepa<a href="$scripturl?action=imsend;caller=$callerid;num=$counter;reply=1;to=$rname;id=$messageid">$inmes_txt{'146'}</a>$sepa<a href="$scripturl?action=imremove;caller=$callerid;id=$messageid" onclick="return confirm('$inmes_txt{'739'}')">$inmes_txt{'31'}</a>~;
		} else {
			$quotemenu = qq~<a href="$scripturl?action=imremove;caller=$callerid;id=$messageid" onclick="return confirm('$inmes_txt{'739'}')">$inmes_txt{'31'}</a>~;
		}

		$yymain .= qq~
     </td>
    <td rowspan="2"><input type="checkbox" name="message$counter"  value="1" style="cursor:hand;" /></td>
  </tr>
  <tr><td colspan="5" height="21" >$immessage<br /><br />
...
~;
	...
	if (@dimmessages != 0) {
		$yymain .= qq~<tr><td colspan="4" height="21"><span  class="small"><b>$imbargfx&nbsp;$intext</b></span></td>$movebutton
					  <td><input type="submit" name="imaction" value="$inmes_imtxt{'10'}" /></td></tr>
					  ~;
	}
	$yymain .= qq~
</table>~;
...
sub IMPost {
	if (!$iamadmin && !$iamgmod && ${$uid.$username}{'postcount'} < $numposts) {
		&ModList;
		fopen(FILE, "$vardir/modslist.txt");
		@moderators = <FILE>;
		fclose(FILE);
		foreach $line (@moderators) {
			chomp $line;
			if ($line eq "$username") { $access = 1; next; }
		}
		if ($access != 1) { &fatal_error($inmes_imtxt{'74'}); }
	}
	...
	<table id="board-head"> 
        <tr> 
          <td valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=im">$inmes_txt{316}</a></b></span></td> 
          <td valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imoutbox">$inmes_txt{320}</a></b></span></td> 
          <td valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imstorage">$inmes_imtxt{46}</a></b></span></td> 
          <td valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=imsend">$inmes_txt{148}</a></b></span></td> 
          <td valign="bottom" width="20%"><span class="small"><b><a href="$scripturl?action=profileCheck;username=$username">$inmes_txt{765}</a></b></span></td>
        </tr> 
      </table><br />
~;
...
sub doshowims {
	...
	if ($INFO{'num'} ne "") {
		...
		&ToChars($msub);
		$yymain .= qq~
	<table width="90%"  id="board">
	    <tr><td>
		<table id="board" cellspacing="0" cellpadding=2 width="100%" align=center>
	<tr><td colspan=2><b>$inmes_txt{'70'}: $msub</b></td></tr>
	~;
		...
		&ToChars($message);
		$yymain .= qq~
	<tr><td ><span class="small">$inmes_txt{'318'}: $musername</span></td><td ><span class="small">$inmes_txt{'30'}: $tempdate</span></td></tr>
	<tr><td colspan="2"><span class="small">$message</span></td></tr></table></td></tr></table>\n
	~;
