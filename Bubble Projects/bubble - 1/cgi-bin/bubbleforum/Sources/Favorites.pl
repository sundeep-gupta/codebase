...
sub Favorites {
	...
	$curboardurl = $curposlinks ?
	                  qq~<a href="$scripturl?board=$currentboard"
		             class="nav">$boardname</a>~
			: $boardname;

	my $homelink = qq~<a href="$scripturl" class="nav">$mbname</a>~;
	my $catlink  = qq~<a href="$scripturl?action=favorites" class="nav">$img_txt{'70'}</a>~;
	$colspan     = 7;
	$markalllink = qq~<a href="$scripturl?board=$INFO{'board'};action=markasread">
			$img{'markboardread'}</a>~ unless ($iamguest) ;
	$postlink    = qq~<a href="$scripturl?board=$INFO{'board'};action=post;title=StartNewTopic">
			$img{'newthread'}</a>~ if (&AccessCheck($currentboard, 1) eq "granted")  ;
	$polllink    = qq~<a href="$scripturl?board=$INFO{'board'};action=post;title=CreatePoll">
			$img{'createpoll'}</a>~ if (&AccessCheck($currentboard, 3) eq "granted") ;
