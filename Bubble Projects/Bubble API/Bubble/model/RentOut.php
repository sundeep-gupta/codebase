...
class RentOutHandler extends Property {
    protected $presenter;
    protected $step;
    protected $form_type;
	...
	if ($category == '2') {
	    ...
	    $row['txn_type'] = $this->get_transaction_id('Rent');;
	    ...
	    $contact         = $this->get_contact_from_id($row['contact']);
	    $row             = $this->get_descriptive_commercial( $row);
	}
	...
	Email::property_added_for_rent($row[0], $contact);
