...
class RequirementView extends HTMLPage {
    public function __construct($model) { 
	$index_page  = new RequirementBody($model);
...
class RequirementBody extends RealEstateBody {
    public function __construct($model) {
	$this->main_body = new Requirement($model);
	parent::__construct( array('id' =>'wrap'));
    }
}

class Requirement extends Div{
    protected $model;
	...
	$data = '';
	if ($step == 1) {
	    $form = new AddressBookForm('controller.php?action=Requirement&event=property_form', 'Personal Details (Step 1 of 2)');
		...
		$form     = new CommercialPropertyForm('controller.php?action=Requirement&event=send',
						       'Requirement Details (Step 2 of 2)', $type,$internet, $lift,
							array('controller.php?action=Requirement&event=property_form_commercial',
							      'controller.php?action=Requirement&event=property_form_residential'));
		...
		$lift      = $this->model->get_lift();
		$form     = new ResidentialPropertyForm('controller.php?action=Requirement&event=send',
							'Requirement Details (Step 2 of 2)', $type,$status, $furnished, $lift,
							array('controller.php?action=Requirement&event=property_form_commercial',
							      'controller.php?action=Requirement&event=property_form_residential'));
	    }
	    $data    .= $form->get_html_text();
