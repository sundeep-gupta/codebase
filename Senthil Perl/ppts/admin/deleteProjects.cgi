...
sub confirm_delete
{
    my $template = HTML::Template->new(filename=>'confirmProject.tmpl');
    my $id = '';
    my $name = '';
    ...
    my $project_string = "SELECT ProjectID, ProjectName
		       FROM projects WHERE ProjectID ='$chosen_project'";
    my $project_command = $db_connection->prepare($project_string);
    $project_command->execute or db_die($db_connection->errstr);
    $project_command->bind_columns(undef, \$id, \$name);
...
sub delete_worker 
{
    ...
    #create sql string and execute it (Client)
    my $delete_project = "DELETE FROM projects WHERE ProjectID = '$id'";
    my $project_command = $db_connection->prepare($delete_project);
    $project_command->execute or db_die($db_connection->errstr);
