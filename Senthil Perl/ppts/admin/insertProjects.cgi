#!/usr/bin/perl -w
######################################################
#                 insertProjects.cgi - PPTS
######################################################
# Copyright   :   (c)2000 Whetstone Logic, Inc.
# Date Started:   October 20, 2000
# Description :   This script inserts projects to 
#                 the projects table on the PPTS DB.
...
{
        error('Inserting new project was not possible.<br>Please contact administrator.');
}
...
# Description: creates DIR for UPLOADING DOCS
###########################
...
sub create_dir
{
	...
	my $client_id = $form_data{ClientID};
        ...
        mkdir($download_dir."/".$project_name."_".$client_id, 0755) or die("Can't create directory $download_dir");
...
sub insert_data
{
	...
	#check parameter
        ...
        my $insert_string = "INSERT INTO projects SET ";

        foreach my $field_name (keys %form_fields)
        {
            
              if (($field_name eq 'EstDesignTime') or ($field_name eq 'EstCodingTime') or
              ($field_name eq 'EstManagementTime') or ($field_name eq 'EstTestingTime') or
              ($field_name eq 'EstMaintanenceTime'))
              {
                   $form_fields{$field_name} = $form_fields{$field_name}.":00:00";
              }
