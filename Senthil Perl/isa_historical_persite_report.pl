...
my $sth = $dbh->prepare("SELECT clientusername,sum(bytesrecvd+bytessent) as val FROM ConsolidatedUserWebProxyLog where DestHost = '$site' and logdate <= '$end_date 23:59:59' and logdate >= '$start_date 00:00:00' group by ClientUserName order by val desc limit 20");
...
$c->addTitle("Top 20 People Visited the site $site - $start_date to $end_date", "arialbi.ttf");
...
<a  href="http://172.25.10.121/cgi-bin/isa_historical_database_report.pl?data=site&site=$site&idate=$start_date&rdate=$end_date"><img src="http://172.25.10.121/excel.gif" border="0" align="top" width="16" height="13" ></a>
<map name="map1">
$imageMap
</map>
</td></tr>
