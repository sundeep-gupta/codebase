...
my $sth = $dbh->prepare("SELECT DestHost,sum(bytesrecvd+bytessent) as val FROM ConsolidatedUserWebProxyLog where clientusername like '%$empid' and logdate <= '$end_date 23:59:59' and logdate >= '$start_date 00:00:00' group by DestHost order by val desc limit 20");
...
$c->addTitle("Top 20 Sites Visited by $user - $start_date to $end_date", "arialbi.ttf");
...
my $imageMap = $c->getHTMLImageMap("isa_historical_persite_report.pl","site={xLabel}&idate=$start_date&rdate=$end_date" ,"title='{xLabel}: {value|0}'");
...
<a href="http://172.25.10.121/cgi-bin/isa_historical_database_report.pl?data=user&empid=$empid&idate=$start_date&rdate=$end_date"><img src="http://172.25.10.121/excel.gif" border="0" align="top" width="16" height="13" ></a>
<map name="map1">
$imageMap
</map>
</td></tr>
