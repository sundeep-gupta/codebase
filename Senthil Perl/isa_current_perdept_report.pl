...
my $sth = $dbh->prepare("SELECT clientusername, sum(bytesrecvd+bytessent) as val FROM DailyUserWebProxyLog where deptid = $deptid group by clientusername order by val desc limit 30");
# SELECT DailyUserWebProxyLog.clientusername, sum(DailyUserWebProxyLog.bytesrecvd+DailyUserWebProxyLog.bytessent) as val FROM DailyUserWebProxyLog JOIN employees1 ON employees1.empid = DailyUserWebProxyLog.clientusername where employees1.deptid = $deptid group by DailyUserWebProxyLog.clientusername order by val desc limit 30");
...
$c->addTitle("Internet Users of $dept - Today", "arialbi.ttf");
...
<a href="http://172.25.10.121/cgi-bin/isa_current_database_report.pl?data=dept&deptid=$deptid"><img src="http://172.25.10.121/excel.gif" border="0" align="top" width="16" height="13" ></a><map name="map1">
$imageMap
</map>
</td>
</tr>
