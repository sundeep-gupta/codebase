...
//mysql_query("INSERT INTO task (taskname, type) VALUES('$taskname', '$priority' ) ") or die(mysql_error()); 
//mysql_query("update task set state=1 where id='$taskid'") or die(mysql_error());  

mysql_query("update task set state=0 where id='$taskid'") or die(mysql_error()); 
...
//Start 
	 ...
	 $display_string .= "<div class=\"eight columns\">";
     $display_string .= "Click on <img src=\"images/restore5.png\"> to Restore task back to the task list";
     $display_string .= "</div>";
     
     $display_string .= "<div class=\"fourr columns\">";
...
$result1 = mysql_query("SELECT id,taskname FROM task where type='UrgentImportant' AND email='$email'  AND state=1") 
or die(mysql_error());  
...
while($row = mysql_fetch_array( $result1 )) {
	...
	//$display_string .= "<input type=\"button\" onclick=\"ajaxFunctionDel('$rowid')\" value=\"Add\">";
        $display_string .= "<input type=\"button\" class=\"button_add4\" onclick=\"ajaxFunctionRep('$rowid')\"  >";
	$display_string .= "</form>";
	$display_string .= "</li>";
...
// Get all the data from the "example" table
$result2 = mysql_query("SELECT id,taskname FROM task where type='NUrgentImportant' AND email='$email'  AND state=1") 
or die(mysql_error());  
...
// Get all the data from the "example" table
$result3 = mysql_query("SELECT id,taskname FROM task where type='UrgentNImportant' AND email='$email'  AND state=1") 
or die(mysql_error());  
...
// Get all the data from the "example" table
$result4 = mysql_query("SELECT id,taskname FROM task where type='NUrgentNImportant' AND email='$email'  AND state=1") 
or die(mysql_error());  
