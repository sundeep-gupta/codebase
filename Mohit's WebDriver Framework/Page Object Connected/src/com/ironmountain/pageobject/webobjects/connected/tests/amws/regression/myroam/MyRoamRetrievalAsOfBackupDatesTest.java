...
public class MyRoamRetrievalAsOfBackupDatesTest extends AccountManagementTest {
	...
	public void testMyRoamRetrievalAsOfBackupDates(String emailid, String password) throws Exception
	{
		...
		myRoamPage.selectVersionOption("As of Backup Date...");	
		String[] datesList = myRoamPage.getBackupDatesList();
		if(datesList.length < 2){
			Asserter.fail("The Backup Dates found in  MyRoam Page is Wrong, There should be atleast 2 Backups!!!");
		}
		/*
		 * Checking the First Backup (We should see only 1 file, which the Base File)
		 */
		myRoamPage.selectBackUpDate(datesList[1]);
		...
		if(fileList.size() != 1){
			Asserter.fail("The revisions of '" + TEST_FILE_NAME + "' found in  MyRoam Page is Wrong!!!");
		}			
		...
		/*
		 * Cleanup the downloaded files for next download
		 */
		FileUtils.deleteDir(retrivedFile);
		FileUtils.deleteDir(retrivedFileUnzipPath);
		FileUtils.createDirectory(FileUtils.getBaseDirectory() +File.separator + PropertyPool.getProperty("downloadsdir"));
		/*
		 * Checking the Second Backup (We should see the New File as well as the delta File)
		 */
		myRoamPage.clickOnBackToSummary();
		myRoamPage = AccountManagementNavigation.viewMyRoamPage();
		logger.info("Verifying the default show version label..");
		myRoamPage.selectVersionOption("As of Backup Date...");	
		myRoamPage.selectBackUpDate(datesList[0]);
		logger.info("Clicking on find button..");
		myRoamPage.clickOnFindBtn();
		logger.info("Typing text..'" + TEST_FILE_NAME + "' for search");
		myRoamPage.enterSearchText(TEST_FILE_NAME);
		myRoamPage.clickOnFindNextBtn();
		logger.info("Check the files from right pane..");
		fileList = myRoamPage.getAllBackedupItemsFromRightPane();
		if(fileList.size() != 2){
			Asserter.fail("The revisions of '" + TEST_FILE_NAME + "' found in  MyRoam Page is Wrong!!!");
		}		
		myRoamPage.checkAllBackupFoldersFromRightPane();
		...
		Asserter.assertTrue(FileUtils.compareFiles(originalFileNew , downloadedFileNew), "Files are not equal!! Compared Files are, Retrieved File : " + downloadedFileDelta + " Original File: " + originalFileDelta);
