	...
	while (st1.hasMoreTokens()) 
		{
			...
			while(st.hasMoreTokens())
			{
					...
					out.println("<TD><input type=\"checkbox\" name=\"chk_file\" value=\""+str1[i]+"|"+str2[i]+"\"></TD>");
					i++;
					k++;					
					j = 1;
					out.println("</TR>");
...
public String[] getfname(String result)throws Exception
{
	int i = 1;
	int j = 1;
	int k = 1;
...
public void getfiles_download(String[] bid_fname,String userInput1,ServletOutputStream out,HttpServletResponse response)
{
		...
		System.out.println(bid_fname[i]);
			...
			try
			{
				do
				{
					StringTokenizer st1 = new StringTokenizer(bid_fname[i],"|");
					while (st1.hasMoreTokens()) 
					{
						bid[i] = st1.nextToken();
...
public void download(String[] bid,String userInput1,ServletOutputStream out,String[] fname,HttpServletResponse response,int j)
{
	try
	{
		String zfile = "C:\\Data\\Tomcat 4.1\\webapps\\examples\\ftpserver\\Files.zip";
		FileOutputStream dest = new FileOutputStream(zfile);
		ZipOutputStream zout = new ZipOutputStream(  new BufferedOutputStream(dest)  );

		for (int k=0 ; k<j ; k++)
		{
			doDataPort_download("retr $$ id="+userInput1+" bid=#"+bid[k]+"", true, is, os,out,fname[k],response,zout);
		}
		zout.close();
		filedownload("Files.zip",out,response);
	}
	catch (Exception e)
	{
		System.out.println(e);
...
public void doDataPort_download(String command, boolean saveToFile, DataInputStream incontrolport, PrintStream outcontrolport,ServletOutputStream out,String fname,HttpServletResponse response,ZipOutputStream zout)
{ 
	ServerSocket serverSocket = null; 
	try 
	{ 
	...
	{ 
		...
		{ 
			...
			file_zip(is,out,fname,response,zout);
...
public void file_zip(InputStream ip,ServletOutputStream out,String fname,HttpServletResponse response,ZipOutputStream zout) throws IOException
{
	try
	{
		BufferedInputStream origin = new BufferedInputStream(ip);
		String file = fname.trim();
	    ...
	    int bytesRead;
	    while ((bytesRead = origin.read(buf)) != -1) zout.write(buf, 0, bytesRead);	
		origin.close();
	}
	catch(Exception e)
	{
...
public void filedownload(String zfile,ServletOutputStream out,HttpServletResponse response) throws IOException
{
	response.setContentType("application/octet-stream");
    response.setHeader ("Content-Disposition", "attachment; filename=\"" + zfile + "\"");
	FileInputStream fis = null;
    		fis = new FileInputStream("C:\\Data\\Tomcat 4.1\\webapps\\examples\\ftpserver\\"+zfile+"");
